                              ...
                              float *norms1, float *norms2, int *info )
{
    ...
    task = QUARK_Task_Init( quark, CORE_cgeqp3_pivot_quark, task_flags );

    QUARK_Task_Pack_Arg( quark, task, sizeof(PLASMA_desc),                  &A,      VALUE          );
    QUARK_Task_Pack_Arg( quark, task, sizeof(PLASMA_Complex32_t)*A.nb*A.nb, F,               INOUT  );
    ...
    for( jj2 = jj; jj2 < A.nt; ++jj2 ) {
        for( ii = 0; ii < A.mt; ++ii ) {
            QUARK_Task_Pack_Arg( quark, task, sizeof(PLASMA_Complex32_t)*A.nb*A.nb, A(ii,jj), INOUT );
        }
    }
    ...
    for( ii = 1; ii < A.nt; ++ii ) {
        QUARK_Task_Pack_Arg( quark, task, sizeof(PLASMA_Complex32_t)*A.nb*A.nb, &F[ii*A.nb], INOUT );
    }

    /* depends on whole norms1 vector (and implicitly, norms2 vector) */
    for( ii = 1; ii < A.nt; ++ii ) {
        QUARK_Task_Pack_Arg( quark, task, sizeof(PLASMA_Complex32_t)*A.nb, &norms1[ii*A.nb], INOUT );
    }
...
#pragma weak CORE_cgeqp3_pivot_quark = PCORE_cgeqp3_pivot_quark
#define CORE_cgeqp3_pivot_quark PCORE_cgeqp3_pivot_quark
#endif
void CORE_cgeqp3_pivot_quark( Quark *quark )
{
    PLASMA_desc A;
    PLASMA_Complex32_t *F;
    int ldf, jj, k;
    int *jpvt;
    float *norms1, *norms2;
    int *info;
    ...
    CORE_cgeqp3_pivot(          A, F, ldf, jj, k, jpvt, norms1, norms2, info );
}
