 ...
 *  Non-blocking equivalent of PLASMA_cLapack_to_Tile().  May return before the
                                 ...
                                 PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if ( A->lm != LDA ){
        plasma_error("PLASMA_cLapack_to_Tile_Async",
    ...
    if ((A->mat == Af77) || (A->mat == NULL) || (Af77 == NULL)) {
        ...
        PLASMA_cgecfi_Async( A->lm, A->ln, A->mat,
                             PlasmaCM, A->lm, 1,
 ...
 *  Non-blocking equivalent of PLASMA_cTile_to_Lapack().  May return before the
 *  computation is finished.
 *  Allows for pipelining of operations ar runtime.
 *
 *******************************************************************************
                                 ...
                                 PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if ( A->lm != LDA ){
        plasma_error("PLASMA_cTile_to_Lapack_Async",
                     "The leading dimension of the output matrix must be equal to the full matrix A->lm");
        return PLASMA_ERR_ILLEGAL_VALUE;
    }
