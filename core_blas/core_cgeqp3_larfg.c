 ...
 *
 * CORE_cgeqp3_larfg generates a Householder elementary reflector H, such that
 *
...
#pragma weak CORE_cgeqp3_larfg = PCORE_cgeqp3_larfg
#define CORE_cgeqp3_larfg PCORE_cgeqp3_larfg
#endif
                        ...
                        PLASMA_Complex32_t *tau, PLASMA_Complex32_t *beta )
{
    int i2, kk, k, mb, lda;
    float norm2;
    PLASMA_Complex32_t x0, scale;
    PLASMA_Complex32_t *Akj, *Aij;
    ...
    for( kk = ii; kk < A.mt; ++kk ) {
        ...
        Akj = A(kk,jj);
        for( k = i2; k < mb; ++k ) {
            norm2 += Akj[k + j*lda] * conjf( Akj[k + j*lda] );
        }
        i2 = 0;
    }
    ...
    if ( norm2 == 0. && cimagf(x0) == 0. ) {
        ...
        *beta = sqrt( crealf(x0)*crealf(x0) + cimagf(x0)*cimagf(x0) + norm2 );
        if ( crealf(x0) >= 0. ) {
            *beta = -(*beta);
        }
        *tau  = (*beta - x0) / crealf(*beta);
