 ...
 * @ingroup CORE_float
 *
 *  CORE_spltmg_chebvand is a kernel used in Vandermonde-like matrix generation
 *
...
#pragma weak CORE_spltmg_chebvand = PCORE_spltmg_chebvand
#define CORE_spltmg_chebvand PCORE_spltmg_chebvand
#endif
                          ...
                          float *W )
{
    ...
    if (m0 == 0) {
        ...
        if ( M == 1 ) {
            LAPACKE_slacpy_work( LAPACK_COL_MAJOR, 'A',
                                 1, N, W, 2, A, LDA );
    ...
    if ( M == 1 ) {
        cblas_scopy( N, W+1, 2, W, 2 );
        cblas_scopy( N, A+M-1, LDA, W+1, 2 );
    }
    else {
        LAPACKE_slacpy_work( LAPACK_COL_MAJOR, 'A',
                             2, N, A + M - 2, LDA, W, 2 );
