 ...
 * @ingroup CORE_double
 *
 *  CORE_dbrdalg1 is a part of the bidiagonal reduction algorithm (bulgechasing).
 ...
 *
 * @param[in] work
 *          Workspace of size nb. Used by the core_dgbtype[123]cb.
 *
 ******************************************************************************/
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_dbrdalg1 = PCORE_dbrdalg1
#define CORE_dbrdalg1 PCORE_dbrdalg1
#endif
void CORE_dbrdalg1( PLASMA_enum uplo, int n, int nb,
                    double *A, int lda,
                    ...
                    double *work)
{
    ...
    for (k = 0; k < grsiz; k++){
        ...
        if( myid == 1 ){
           CORE_dgbtype1cb(uplo, n, nb, A, lda, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1,  Vblksiz, wantz, work);
        }else if(myid%2 == 0){
           CORE_dgbtype2cb(uplo, n, nb, A, lda, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, wantz, work);
        }else{ /*if(myid%2 == 1)*/
           CORE_dgbtype3cb(uplo, n, nb, A, lda, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, wantz, work);
        }
