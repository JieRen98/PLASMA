 ...
 *
 *  CORE_clacpy_pivot extracts the original version of the rows selected by the
...
#pragma weak CORE_clacpy_pivot = PCORE_clacpy_pivot
#define CORE_clacpy_pivot PCORE_clacpy_pivot
#endif
                       ...
                       int init)
{
    ...
    if (direct == PlasmaRowwise) {
       ...
       for(i=k1-1; i<k2; i++, ro++) {
           ...
           ir = ip % descA.mb;
           ld = BLKLDD(descA, it);
           cblas_ccopy(descA.n, A(it, 0) + ir, ld,
                                A + i,         lda );
       }
    }
    else {
