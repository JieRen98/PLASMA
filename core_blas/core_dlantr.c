 ...
 *          If uplo == PlasmaUpper, M <= N. When M = 0, CORE_dlantr returns 0.
 *
 * @param[in] N
 *          The number of columns of the matrix A. N >= 0.
 *          If uplo == PlasmaLower, N <= M. When N = 0, CORE_dlantr returns 0.
 *
 * @param[in] A
...
#pragma weak CORE_dlantr = PCORE_dlantr
#define CORE_dlantr PCORE_dlantr
#endif
                 ...
                 double *work, double *normA)
{
    ...
    *normA = LAPACKE_dlantr_work(
    ...
    case PlasmaOneNorm:
        CORE_dtrasm( PlasmaColumnwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == PlasmaLower )
            N = min(M,N);
    ...
    case PlasmaInfNorm:
        CORE_dtrasm( PlasmaRowwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == PlasmaUpper )
            M = min(M,N);
