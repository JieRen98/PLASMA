...
static inline void
CORE_dgetrf_rectil_rec(CORE_dgetrf_data_t *data,
                       ...
                       int ft,     int lt);
static inline void
CORE_dgetrf_rectil_update(CORE_dgetrf_data_t *data,
 ...
 *          CORE_dgetrf_rectil_init() that contains information for thread
                       ...
                       const PLASMA_desc A, int *IPIV, int *info)
{
    ...
    CORE_dgetrf_rectil_rec( data, A, IPIV, info, &pivot,
                            thidx, thcnt, 0, minMN, ft, lt);

    if ( A.n > minMN ) {
        CORE_dgetrf_rectil_update( data, A, IPIV,
                                   0, minMN, A.n-minMN,
                                   thidx, thcnt,
                                   ft, lt);
    }
...
CORE_dgetrf_data_t *
CORE_dgetrf_rectil_init(int nbthrd)
{
    int i;
                   ...
                   int pividx, int *ipiv)
{
    ...
    if (thidx == 0) {
        ...
        CORE_damax[0] = *diagvalue; /* set the index of the winning thread */
                     ...
                     int thidx, int thcnt)
{
    ...
    CORE_damax1_thread( data, 1.0, thidx, thcnt, &idum1, &ddum1, &ddum2, 0, &idum2 );
}
                       ...
                       int ft,     int lt)
{
    ...
    if ( width > 1 ) {
        ...
        CORE_damax1_thread( data, tmp1, thidx, thcnt, &thwin,
