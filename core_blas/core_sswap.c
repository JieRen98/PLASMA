 ...
 * @ingroup CORE_float
 *
 *  CORE_sswap - Extract the eigenvectors in the range [start,end-1]
...
#pragma weak CORE_sswap = PCORE_sswap
#define CORE_sswap PCORE_sswap
#endif
void CORE_sswap(int m, int n,
                float *Q, int ldq,
                const float *work,
                const int *perm,
                int start, int end)
{
    ...
    for (i=start; i<end; i++, Q+=ldq){
        cblas_scopy(m, work+m*perm[i], 1, Q, 1);
    }
}
