 ...
 * @ingroup CORE_float
 *
 *  CORE_sgetf2_nopiv computes an LU factorization of a general diagonal
 *  dominant M-by-N matrix A witout no pivoting and no blocking. It is the
 *  internal function called by CORE_sgetrf_nopiv().
 *
        ...
        if ( alpha != (float)0.0 ) {
            ...
            if (i < M) {
                if ( fabsf(alpha) > sfmin ) {
                    alpha = 1.0 / alpha;
                    cblas_sscal( M-i-1, (alpha), &(A[i*LDA+i+1]), 1);
                } else {
                    for(j=i+1; j<M; j++)
