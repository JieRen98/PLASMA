 ...
 * @ingroup CORE_float
 *
 *  CORE_slascal scales a two-dimensional matrix A. As opposite to
 *  CORE_slascl(), no checks is performed to prevent under/overflow. This should
 ...
 ******************************************************************************/
int
CORE_slascal( PLASMA_enum uplo, int m, int n,
              float alpha, float *A, int lda )
{
    ...
    case PlasmaUpper:
        for(i=0; i<n; i++) {
            cblas_sscal( min( i+1, m ), (alpha), A+i*lda, 1 );
        }
        break;

    case PlasmaLower:
        for(i=0; i<n; i++) {
            cblas_sscal( max( m, m-i ), (alpha), A+i*lda, 1 );
        }
        break;
    default:
        if (m == lda) {
