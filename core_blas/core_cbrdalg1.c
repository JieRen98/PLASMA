 ...
 *
 *  CORE_cbrdalg1 is a part of the bidiagonal reduction algorithm (bulgechasing).
 ...
 *
 * @param[in,out] A
 *          PLASMA_Complex32_t array, dimension (lda,n)
 ...
 *
 * @param[out] VQ
 *          PLASMA_Complex32_t array, dimension (n) if wantz=0
 ...
 *
 * @param[in] work
 *          Workspace of size nb. Used by the core_cgbtype[123]cb.
 *
 ******************************************************************************/
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_cbrdalg1 = PCORE_cbrdalg1
#define CORE_cbrdalg1 PCORE_cbrdalg1
#endif
void CORE_cbrdalg1( PLASMA_enum uplo, int n, int nb,
                    ...
                    PLASMA_Complex32_t *work)
{
    ...
    for (k = 0; k < grsiz; k++){
        ...
        if( myid == 1 ){
           CORE_cgbtype1cb(uplo, n, nb, A, lda, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1,  Vblksiz, wantz, work);
        }else if(myid%2 == 0){
           CORE_cgbtype2cb(uplo, n, nb, A, lda, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, wantz, work);
        }else{ /*if(myid%2 == 1)*/
           CORE_cgbtype3cb(uplo, n, nb, A, lda, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, wantz, work);
        }
