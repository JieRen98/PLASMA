 ...
 *
 *  CORE_claswp performs a series of row interchanges on the matrix A.
 ...
 * @ingroup CORE_PLASMA_Complex32_t
 *
 *  CORE_claswpc_ontile apply the claswp function on a matrix stored in
 *  tile layout
 *
 *******************************************************************************
 *
 ...
 */
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_claswpc_ontile = PCORE_claswpc_ontile
#define CORE_claswpc_ontile PCORE_claswpc_ontile
#endif
    ...
    if (inc > 0) {
        ...
        for (j = i1-1; j < i2; ++j, ipiv+=inc) {
            ...
            if ( ip != j )
            {
                it = ip / descA.nb;
                i  = ip % descA.nb;
                cblas_cswap(descA.m, A1       + j*lda, 1,
                                     A(0, it) + i*lda, 1 );
            }
        }
    }
