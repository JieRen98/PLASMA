 ...
 * @ingroup CORE_double
 *
 *  CORE_dlascal scales a two-dimensional matrix A. As opposite to
 *  CORE_dlascl(), no checks is performed to prevent under/overflow. This should
 ...
 ******************************************************************************/
int
CORE_dlascal( PLASMA_enum uplo, int m, int n,
              double alpha, double *A, int lda )
{
    ...
    case PlasmaUpper:
        for(i=0; i<n; i++) {
            cblas_dscal( min( i+1, m ), (alpha), A+i*lda, 1 );
        }
        break;

    case PlasmaLower:
        for(i=0; i<n; i++) {
            cblas_dscal( max( m, m-i ), (alpha), A+i*lda, 1 );
        }
        break;
    default:
        if (m == lda) {
