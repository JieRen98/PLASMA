...
#pragma weak CORE_dpltmg_chebvand = PCORE_dpltmg_chebvand
#define CORE_dpltmg_chebvand PCORE_dpltmg_chebvand
#endif
                          ...
                          double *W )
{
    ...
    if ( M == 1 ) {
        cblas_dcopy( N, W+1, 2, W, 2 );
        cblas_dcopy( N, A+M-1, LDA, W+1, 2 );
    }
    else {
        LAPACKE_dlacpy_work( LAPACK_COL_MAJOR, 'A',
                             2, N, A + M - 2, LDA, W, 2 );
