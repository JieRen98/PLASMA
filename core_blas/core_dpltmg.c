 ...
 * @ingroup CORE_double
 *
 *  CORE_dpltmg initialize a tile of a random matrix from the MatLab
...
#pragma weak CORE_dpltmg = PCORE_dpltmg
#define CORE_dpltmg PCORE_dpltmg
...
#endif
int CORE_dpltmg( PLASMA_enum mtxtype,
                  ...
                  unsigned long long int seed )
{
    ...
    {
        CORE_dplrnt( M, N, A, LDA, gM, m0, n0, seed );
    }
    break;
    ...
    case PlasmaMatrixParter:
    {
        ...
        for (j=0; j<N; j++) {
            for (i=0; i<M; i++) {
                A[j*LDA+i] = (double)1. / (double)( tmp + i - j );
            }
        }
    }
    break;
     ...
     case PlasmaMatrixRis:
    {
        ...
        tmp = (double)( gM - m0 - n0 - 0.5 );
        for (j=0; j<N; j++) {
            for (i=0; i<M; i++) {
                A[j*LDA+i] = (double).5 / (double)( tmp - i - j );
            }
        }
    }
    break;
    ...
    case PlasmaMatrixKms:
    {
        ...
        for (j=0; j<N; j++) {
            for (i=0; i<M; i++) {
                A[j*LDA+i] = (double)( pow( rho, fabs( (double)( m0 + i - n0 - j ) ) ) );
            }
        }
    }
    break;
    ...
    case PlasmaMatrixMoler:
    {
        ...
        for (j=0,jj=n0; j<N; j++,jj++) {
            for (i=0,ii=m0; i<M; i++,ii++) {
                if ( ii == jj ) {
                    A[j*LDA+i] = (double)( ii + 1. );
                } else {
                    A[j*LDA+i] = (double)( min( ii, jj ) - 1. );
                }
            }
        }
    }
    ...
    case PlasmaMatrixCompan:
    {
        ...
        LAPACKE_dlaset_work(LAPACK_COL_MAJOR, 'A', M, N, 0., 0., A, LDA);

        /* First row */
        if ( m0 == 0 ) {
            double v0;
            /* Get V0 */
            CORE_dplrnt( 1, 1, &v0, 1, 1, 1, 0, seed );
            v0 = 1. / v0 ;

            /* Initialize random vector */
            CORE_dplrnt( 1, N, A, LDA, 1, 1, n0, seed );
            cblas_dscal( N, (v0), A, LDA);
     ...
     case PlasmaMatrixLehmer:
    {
        ...
        for (j=0,jj=n0+1; j<N; j++,jj++) {
            for (i=0,ii=m0+1; i<M; i++,ii++) {
                if ( jj >= ii ) {
                    A[j*LDA+i] = (double)( ii ) / (double)( jj );
                } else {
                    A[j*LDA+i] = (double)( jj ) / (double)( ii );
                }
            }
        }
    }
    ...
    case PlasmaMatrixCauchy:
    {
        ...
        for (j=0,jj=n0+1; j<N; j++,jj++) {
            for (i=0,ii=m0+1; i<M; i++,ii++) {
                A[j*LDA+i] = (double)( 1. ) / (double)( ii+jj );
            }
        }
    }
    break;
    ...
    {
        double sqrtn = (double) sqrt( 2. / (gN+1.) );
        double scale = pi / (double)(gN+1.);

        int ii, jj;
        for (j=0,jj=n0+1; j<N; j++,jj++) {
            for (i=0,ii=m0+1; i<M; i++,ii++) {
                A[j*LDA+i] = sqrtn * (double) sin( (double)ii * (double)jj * scale );
            }
        }
    }
    break;
   ...
   case PlasmaMatrixWilkinson:
    {
        ...
        for (j=0,jj=n0; j<N; j++,jj++) {
            for (i=0,ii=m0; i<M; i++,ii++) {
                if (ii == jj) {
                    double tmp = (double)(( (gN - 1 - ii) < ii ) ? gN - 1 - ii : ii );
                     A[j*LDA+i] = (double)(gN - 2. * tmp - 1.) / 2.;
                }
            ...
            for (i=0,ii=m0; i<M; i++,ii++) {
                ...
                if (ii == jj) {
                    if (jj == 0)
                        ...
                        A[j*LDA+i] = (double)(1. - (1. / c) - (k*h)/2. );
                    else
                        A[j*LDA+i] = (double)(1. - (k*h)/2. );
                }
                else if (jj == 0) {
                    A[j*LDA+i] = (double)(-k*h/2.);
                }
                else if (jj == gN-1) {
                    A[j*LDA+i] = (double)(-1./c);
                }
                else if (ii > jj) {
                    A[j*LDA+i] = (double)(-k*h);
                }
                else {
                    A[j*LDA+i] = (double)0.;
                }
        ...
        for (j=0,jj=n0; j<N; j++,jj++) {
            for (i=0,ii=m0; i<M; i++,ii++) {
                ...
                if (ii == jj)
                    ...
                    A[j*LDA+i] = (double)(-0.9048);
                else if ((ii == jj + 3) && (jj % 2 == 0))
                    A[j*LDA+i] = (double)(-1.2092);
                else if ((ii == jj + 2) && (jj % 2 == 1))
                    A[j*LDA+i] = (double)(-0.8270);
                else if ((ii == jj + 3) && (jj % 2 == 1))
                    A[j*LDA+i] = (double)(-1.3499);
    ...
    {
        double eps = (double) LAPACKE_dlamch_work( 'e' );
        int ii, jj, mm, minMN;
        int firstcol, lastcol;
         ...
         if ( (m0 >= n0) && ((n0+N) >= firstcol ) && (n0 < lastcol) ) {
             ...
             for (; j<N && jj < lastcol; j++,jj++) {
                 ...
                 i  = ii - m0;
                 mm = M - i;
                 cblas_dscal( mm, (eps), A + j*LDA + i, 1);
             }
         }
    }
    break;
