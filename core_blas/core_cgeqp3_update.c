 ...
 *
 * CORE_cgeqp3_update updates row k of one tile of A
 ...
 *         in updating column norms. cgeqp3 handles this error.
 **/
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_cgeqp3_update = PCORE_cgeqp3_update
#define CORE_cgeqp3_update PCORE_cgeqp3_update
#endif
                         ...
                         int *info )
{
    ...
    const PLASMA_Complex32_t zone  =  1.0;
    const PLASMA_Complex32_t mzone = -1.0;
    int j;

    /* update row k of A -- this is vector*matrix */
    /* Ajk[k,j:nb] -= Ajj[k,0:k+1] * Fk[j:nb,0:k+1].T */
    cblas_cgemm( CblasColMajor, CblasNoTrans, CblasConjTrans, 1, nb-koff, k+1,
    ...
    for( j = koff; j < nb; ++j ) {
        if ( norms1[j] != 0. ) {
            ...
            temp = cabsf( Ajk[joff+k + j*lda2] ) / norms1[j];
