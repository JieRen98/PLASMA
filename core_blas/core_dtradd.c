 ...
 * @ingroup CORE_double
 *
 *  CORE_dtradd adds two matrices together as in PBLAS pdtradd.
 *
 *       B <- alpha * op(A)  + beta * B,
 *
 * where op(X) = X, X', or (X')
...
#pragma weak CORE_dtradd = PCORE_dtradd
#define CORE_dtradd PCORE_dtradd
#define CORE_dgeadd PCORE_dgeadd
int
CORE_dgeadd(PLASMA_enum trans, int M, int N,
                  double alpha,
                      ...
                      double *B, int LDB)
{
    ...
    if (uplo == PlasmaUpperLower){
        int rc = CORE_dgeadd( trans, M, N, alpha, A, LDA, beta, B, LDB );
