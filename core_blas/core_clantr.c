 ...
 *          If uplo == PlasmaUpper, M <= N. When M = 0, CORE_clantr returns 0.
 *
 * @param[in] N
 *          The number of columns of the matrix A. N >= 0.
 *          If uplo == PlasmaLower, N <= M. When N = 0, CORE_clantr returns 0.
 *
 * @param[in] A
...
#pragma weak CORE_clantr = PCORE_clantr
#define CORE_clantr PCORE_clantr
#endif
                 ...
                 float *work, float *normA)
{
    ...
    *normA = LAPACKE_clantr_work(
    ...
    case PlasmaOneNorm:
        CORE_ctrasm( PlasmaColumnwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == PlasmaLower )
            N = min(M,N);
    ...
    case PlasmaInfNorm:
        CORE_ctrasm( PlasmaRowwise, uplo, diag, M, N,
                     A, LDA, work );
        if ( uplo == PlasmaUpper )
            M = min(M,N);
