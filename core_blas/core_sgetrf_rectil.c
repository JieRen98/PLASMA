...
static inline void
CORE_sgetrf_rectil_rec(CORE_sgetrf_data_t *data,
                       ...
                       int ft,     int lt);
static inline void
CORE_sgetrf_rectil_update(CORE_sgetrf_data_t *data,
 ...
 * @ingroup CORE_float
 *
 *  CORE_sgetrf_rectil computes a LU factorization of a general M-by-N
 ...
 *
 *  WARNINGS:
 *     - The function CORE_sgetrf_rectil_init has to be called prior
 ...
 *          CORE_sgetrf_rectil_init() that contains information for thread
...
#endif
int CORE_sgetrf_rectil(CORE_sgetrf_data_t *data,
                       const PLASMA_desc A, int *IPIV, int *info)
{
    int ft, lt;
    int thidx = info[1];
    ...
    CORE_sgetrf_rectil_rec( data, A, IPIV, info, &pivot,
                            thidx, thcnt, 0, minMN, ft, lt);

    if ( A.n > minMN ) {
        CORE_sgetrf_rectil_update( data, A, IPIV,
                                   0, minMN, A.n-minMN,
                                   thidx, thcnt,
                                   ft, lt);
    }
...
CORE_sgetrf_data_t *
CORE_sgetrf_rectil_init(int nbthrd)
{
    int i;
                   ...
                   int pividx, int *ipiv)
{
    ...
    if (thidx == 0) {
        ...
        CORE_samax[0] = *diagvalue; /* set the index of the winning thread */
                     ...
                     int thidx, int thcnt)
{
    ...
    CORE_samax1_thread( data, 1.0, thidx, thcnt, &idum1, &ddum1, &ddum2, 0, &idum2 );
}
                       ...
                       int ft,     int lt)
{
    ...
    if ( width > 1 ) {
        ...
        CORE_samax1_thread( data, tmp1, thidx, thcnt, &thwin,
