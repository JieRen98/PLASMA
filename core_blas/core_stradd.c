 ...
 * @ingroup CORE_float
 *
 *  CORE_stradd adds two matrices together as in PBLAS pstradd.
 *
 *       B <- alpha * op(A)  + beta * B,
 *
 * where op(X) = X, X', or (X')
...
#pragma weak CORE_stradd = PCORE_stradd
#define CORE_stradd PCORE_stradd
#define CORE_sgeadd PCORE_sgeadd
int
CORE_sgeadd(PLASMA_enum trans, int M, int N,
                  float alpha,
                      ...
                      float *B, int LDB)
{
    ...
    if (uplo == PlasmaUpperLower){
        int rc = CORE_sgeadd( trans, M, N, alpha, A, LDA, beta, B, LDB );
