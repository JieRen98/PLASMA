 ...
 ******************************************************************************/
int
CORE_slascl( PLASMA_enum type, int kl, int ku, float cfrom, float cto,
             int m, int n, float *A, int lda )
{
    ...
    info = LAPACKE_slascl_work( LAPACK_COL_MAJOR, lapack_const(type), kl, ku, cfrom, cto,
                                m, n, A, lda );
#else
    PLASMA_FCALL(slascl, SLASCL)( &(lapack_const(type)), &kl, &ku, &cfrom, &cto,
                                  &m, &n, A, &lda, &info );
#endif

    if (info != 0){
        coreblas_error(info, "numerical error in slascl\n");
    }
