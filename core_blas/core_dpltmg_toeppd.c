...
#pragma weak CORE_dpltmg_toeppd1 = PCORE_dpltmg_toeppd1
#define CORE_dpltmg_toeppd1 PCORE_dpltmg_toeppd1
#endif
                          ...
                          unsigned long long int seed )
{
    ...
    CORE_dplrnt( 2, M, W, 2, gM, 0, m0, seed );
 ...
 * @ingroup CORE_double
 *
 *  CORE_dpltmg_toeppd2 is the first kernel used in toeppd matrix generation.
 *
 *  See http://www.mathworks.fr/fr/help/matlab/ref/gallery.html#f84-1000272
 *
 *  A toeppd matrix is an n-by-n symmetric, positive semi-definite (SPD)
 ...
 ******************************************************************************/
#if defined(PLASMA_HAVE_WEAK)
#pragma weak CORE_dpltmg_toeppd2 = PCORE_dpltmg_toeppd2
#define CORE_dpltmg_toeppd2 PCORE_dpltmg_toeppd2
#endif
    ...
    for (j=0, jj=n0; j<N; j++, jj++) {
        for (i=0, ii=m0; i<M; i++, ii++, A++) {
            ...
            for(k=0; k<K; k++, tmpW+=2) {
                *A += (tmpW[0]) * cos( tmpW[1] * (double)(ii-jj) );
            }
        }
        A += (LDA - M);
    }
