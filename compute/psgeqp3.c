...
static const float zone  = (float)  1.;
static const float mzone = (float) -1.;
static const float zzero = (float)  0.;
                           ...
                           PLASMA_sequence *sequence, PLASMA_request *request )
{
    ...
    QUARK_CORE_sgeqp3_init( plasma->quark, &task_flags, A.n, jpvt );

    /* Compute the initial norm of each column of A */
    for( kk = 0; kk < A.nt; kk++ ) {
        kn = BLKN( A, kk );
        /* set norms2[:] = -1. to force sgeqp3_norms to compute all column norms */
        QUARK_CORE_slaset( plasma->quark, &task_flags,
            PlasmaUpperLower, kn, 1, mzone, mzone, &norms2[kk*A.nb], 1 );
        QUARK_CORE_sgeqp3_norms( plasma->quark, &task_flags,
    ...
    while( j < minmn ) {
        ...
        while( k < jb ) {
            ...
            QUARK_CORE_sgeqp3_pivot( plasma->quark, &task_flags,
            ...
            if ( k > 0 ) {
                ...
                for( ii = jj; ii < A.mt; ++ii ) {
                    ...
                    lda = BLKLDD( A, ii );
                    Aij = A(ii,jj);
                    /* really gemv, but F needs to be ugated, so use gemm */
                    QUARK_CORE_sgemm_tile( plasma->quark, &task_flags,
                        PlasmaNoTrans, PlasmaTrans, im-ioff, 1, k, A.nb,
            ...
            /* Householder */
            QUARK_CORE_sgeqp3_larfg( plasma->quark, &task_flags,
            ...
            if ( k < nj-1 ) {
                ...
                for( ii = jj; ii < A.mt; ++ii ) {
                    ...
                    for( kk = jj; kk < A.nt; ++kk ) {
                        ...
                        Aik = A(ii,kk);
                        Fk  = &F[(kk-jj)*A.nb];
                        QUARK_CORE_sgemv_tile( plasma->quark, &task_flags,
                            PlasmaTrans, im-ioff, kn-koff,
            ...
            for( kk = jj; kk < A.nt; ++kk ) {
                ...
                Ajk = A(jj,kk);
                Fk  = &F[(kk-jj)*A.nb];
                QUARK_CORE_sgeqp3_update( plasma->quark, &task_flags,
            ...
            /* save beta (from slarfg) to diagonal */
            QUARK_CORE_ssetvar( plasma->quark, &task_flags,
        ...
        for( ii = jj; ii < A.mt; ++ii ) {
            ...
            for( kk = jj; kk < A.nt; ++kk ) {
                ...
                if ( k < jb || (ii > jj && kk > jj)) {
                    ...
                    QUARK_CORE_sgemm_tile( plasma->quark, &task_flags,
                        PlasmaNoTrans, PlasmaTrans, im-ioff, kn-koff, jb, A.nb,
                        &mzone, &Aij[ioff + joff*lda], lda,
                                & Fk[koff           ], ldf,
