 ...
 *
 *  PLASMA_cgebrd - reduces a general complex M-by-N matrix A to upper or lower
 ...
 *
 * @param[out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_cgebrd
 ...
 *
 ******************************************************************************/
int PLASMA_cgebrd(PLASMA_enum jobq, PLASMA_enum jobp,
                  int M, int N,
                  ...
                  PLASMA_Complex32_t *P, int LDP)
{
    ...
    if (jobq != PlasmaNoVec  && jobq !=PlasmaVec) {
        plasma_error("PLASMA_cgebrd", "illegal value of jobq");
        return -1;
    }
    if (jobp != PlasmaNoVec && jobp != PlasmaVec) {
        plasma_error("PLASMA_cgebrd", "illegal value of jobp");
        return -2;
    }
    if (M < 0) {
    ...
    if (LDQ < 1) {
        plasma_error("PLASMA_cgebrd", "illegal value of LDQ");
        return -9;
    }
    if (LDP < 1) {
        plasma_error("PLASMA_cgebrd", "illegal value of LDP");
        return -11;
    }
    /* Quick return */
    if (min(M, N) == 0) {
    ...
    PLASMA_cgebrd_Tile_Async(jobq, jobp, &descA, D, E, descT, Q, LDQ, P, LDP, sequence, &request);
 ...
 *
 *  PLASMA_cgebrd_Tile - reduces a general complex M-by-N matrix A to upper or lower
 *  bidiagonal form B using a two-stage approach
 *  First stage: reduction to band bidiagonal form (orthogonal matrices Q1 and P1);
 *  Second stage: reduction from band to bidiagonal form (orthogonal matrices
 *  Q2 and P2).
 ...
 *
 ******************************************************************************/
int PLASMA_cgebrd_Tile(PLASMA_enum jobq, PLASMA_enum jobp,
                       PLASMA_desc *A,
                       float *D, float *E,
                       PLASMA_desc *T,
                       PLASMA_Complex32_t *Q, int LDQ,
                       PLASMA_Complex32_t *P, int LDP)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_cgebrd_Tile_Async(jobq, jobp, A, D, E, T, Q, LDQ, P, LDP, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_cgebrd_Tile_Async - reduces a general complex M-by-N matrix A to upper or lower
 *  bidiagonal form B using a two-stage approach
 *  First stage: reduction to band bidiagonal form (orthogonal matrices Q1 and P1);
 *  Second stage: reduction from band to bidiagonal form (orthogonal matrices
 *  Q2 and P2).
 ...
 *
 ******************************************************************************/
int PLASMA_cgebrd_Tile_Async(PLASMA_enum jobu, PLASMA_enum jobvt,
                             PLASMA_desc *A,
                             float *S, float *E,
                             PLASMA_desc *T,
                             ...
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if( ( M<= NB) || ( N <=NB ) ){
        ...
        PLASMA_Complex32_t *TAUQ = (PLASMA_Complex32_t *) plasma_shared_alloc(plasma, MINMN, PlasmaComplexFloat);
        PLASMA_Complex32_t *TAUP = (PLASMA_Complex32_t *) plasma_shared_alloc(plasma, MINMN, PlasmaComplexFloat);
        status = LAPACKE_cgebrd(LAPACK_COL_MAJOR, M, N, AB, M, S, E, TAUQ, TAUP);
        if(jobu == PlasmaVec){
            LAPACKE_clacpy(LAPACK_COL_MAJOR, lapack_const(PlasmaLower), M, MINMN, AB, M, U, LDU );
            LAPACKE_cungbr(LAPACK_COL_MAJOR, 'Q', M, M, N, U, LDU, TAUQ );
        }
        if(jobvt == PlasmaVec){
            LAPACKE_clacpy(LAPACK_COL_MAJOR, lapack_const(PlasmaUpper), MINMN, N, AB, M, VT, LDVT );
            LAPACKE_cungbr(LAPACK_COL_MAJOR, 'P', N, N, MINMN, VT, LDVT, TAUP );
        }
        if(status != 0){
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_cgebrd_Tile_Async", "plasma_shared_alloc(AB) failed");
        plasma_shared_free(plasma, AB);
        return PLASMA_ERR_OUT_OF_RESOURCES;
    }
    /*=======================================
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgebrd","pcgebrd_ge2gb+pccopy");
        return status;
    }
    ...
    if( jobu == PlasmaVec ) {
        ...
        if ( (TAUQ2 == NULL) || (VQ2 == NULL) || (TQ2 == NULL) ) {
            plasma_error("PLASMA_cgebrd", "plasma_shared_alloc() failed");
