 ...
 * @author Azzam Haidar
 * @date 2010-11-15
 * @generated c Fri Apr  1 11:02:56 2016
 *
 **/
#include <lapacke.h>
#include "common.h"
 ...
 *
 *  PLASMA_cgesdd - computes the singular value decomposition (SVD) of a complex
 ...
 *  LAPACK CGESDD. We have jobu and jobvt instead of only jobz first
 *  to be consistent with CGESVD and second to allow a computation
 *  of only U or VT.
 *******************************************************************************
 *
 * @param[in] jobu
 ...
 *
 * @param[in, out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_cgesdd
 ...
 *
 ******************************************************************************/
int PLASMA_cgesdd(PLASMA_enum jobu, PLASMA_enum jobvt, int M, int N,
                  ...
                  PLASMA_Complex32_t *VT, int LDVT)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgesdd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if (jobu != PlasmaNoVec  && jobu !=PlasmaVec) {
        plasma_error("PLASMA_cgesdd", "illegal value of jobu");
        return -1;
    }
    if (jobvt != PlasmaNoVec && jobvt != PlasmaVec) {
        plasma_error("PLASMA_cgesdd", "illegal value of jobvt");
        return -2;
    }
    if (jobvt != jobu) {
        plasma_error("PLASMA_cgesdd", "in this version: jobu should be equal jobvt");
        return -22;
    }
    if (M < 0) {
        plasma_error("PLASMA_cgesdd", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_cgesdd", "illegal value of N");
        return -4;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_cgesdd", "illegal value of LDA");
        return -6;
    }
    if (LDU < 1) {
        plasma_error("PLASMA_cgesdd", "illegal value of LDU");
        return -9;
    }
    if (LDVT < 1) {
        plasma_error("PLASMA_cgesdd", "illegal value of LDVT");
        return -11;
    }
    /* Quick return */
    if (min(M, N) == 0) {
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgesdd", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_cgesdd_Tile_Async(jobu, jobvt, &descA, S, descT, U, LDU, VT, LDVT, sequence, &request);
 ...
 *
 *  PLASMA_cgesdd_Tile - computes the singular value decomposition (SVD) of a complex
 *  M-by-N matrix A, optionally computing the left and/or right singular
 *  vectors.
 *  Tile equivalent of PLASMA_cgesdd().
 ...
 *
 ******************************************************************************/
int PLASMA_cgesdd_Tile(PLASMA_enum jobu, PLASMA_enum jobvt,
                       PLASMA_desc *A,
                       float *S,
                       PLASMA_desc *T,
                       PLASMA_Complex32_t *U, int LDU,
                       PLASMA_Complex32_t *VT, int LDVT)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgesdd_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_cgesdd_Tile_Async(jobu, jobvt, A, S, T, U, LDU, VT, LDVT, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_cgesdd_Tile_Async - computes the singular value decomposition (SVD) of a complex
 *  M-by-N matrix A, optionally computing the left and/or right singular
 *  vectors.
 *  Non-blocking equivalent of PLASMA_cgesdd_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_cgesdd_Tile_Async(PLASMA_enum jobu, PLASMA_enum jobvt,
                             PLASMA_desc *A,
                             float *S,
                             PLASMA_desc *T,
                             PLASMA_Complex32_t *U, int LDU,
                             ...
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cgesdd_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_cgesdd_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_cgesdd_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(T) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "invalid fourth descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descT = *T;
    }
    /* Check input arguments */
    if (jobu != PlasmaNoVec  && jobu != PlasmaVec) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "illegal value of jobu");
        return PLASMA_ERR_NOT_SUPPORTED;
    }
    if (jobvt != PlasmaNoVec && jobvt != PlasmaVec) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "illegal value of jobvt");
        return PLASMA_ERR_NOT_SUPPORTED;
    }
    if (jobvt != jobu) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "in this version: jobu should be equal jobvt");
        return PLASMA_ERR_NOT_SUPPORTED;
    }
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    if( ( M<= NB) || ( N <=NB ) ){
        ...
        status = LAPACKE_cgesdd(LAPACK_COL_MAJOR, lapack_const(jobz),
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "plasma_shared_alloc(AB) failed");
        plasma_shared_free(plasma, AB);
        return PLASMA_ERR_OUT_OF_RESOURCES;
    }
    E = (float *)plasma_shared_alloc(plasma, MINMN, PlasmaRealDouble);
    if (E == NULL) {
        plasma_error("PLASMA_cgesdd_Tile_Async", "plasma_shared_alloc(E) failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cgesdd","pcgebrd_ge2gb+pccopy");
        return status;
    }
    ...
    */
    ...
    if( jobu == PlasmaVec ) {
        ...
        if ( (TAUQ2 == NULL) || (VQ2 == NULL) || (TQ2 == NULL) ) {
            plasma_error("PLASMA_cgesdd", "plasma_shared_alloc() failed");
    ...
    if((jobu == PlasmaNoVec)&&(jobvt == PlasmaNoVec)){
        ...
        //LAPACKE_clacp2(LAPACK_COL_MAJOR, lapack_const(uplo), MINMN, MINMN, RU, MINMN, U, LDU );
        //LAPACKE_clacp2(LAPACK_COL_MAJOR, lapack_const(uplo), MINMN, MINMN, RVT, MINMN, VT, LDVT );
        for(j=0; j<MINMN; j++){
            for(i=0; i<MINMN; i++){
                U[i+j*LDU] = (PLASMA_Complex32_t) RU[i+j*MINMN];
            }
        }
        for(j=0; j<MINMN; j++){
            for(i=0; i<MINMN; i++){
                VT[i+j*LDVT] = (PLASMA_Complex32_t) RVT[i+j*MINMN];
            }
        }
    ...
    if(status != 0){
        plasma_error("PLASMA_cbdedc","ZBDEDC");
        /*return status;*/
    }
    sequence->status = status;
    plasma_setlapack_sequential(plasma);
    ...
    if (jobu == PlasmaVec){
        ...
        if( NB < N ){
            ...
            if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
                plasma_cooplap2tile( descU, U, NB, NB, LDU, ME, 0, 0, M, ME, sequence, request, plasma_desc_mat_free(&(descU)) );
            } else {
                plasma_ciplap2tile( descU, U, NB, NB, LDU, ME, 0, 0, M, ME, sequence, request);
            }
            ...
            if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
                plasma_cooptile2lap( descU, U, NB, NB, LDU, ME, sequence, request );
                plasma_dynamic_sync();
                plasma_desc_mat_free(&descU);
            } else {
                plasma_ciptile2lap( descU, U, NB, NB, LDU, ME, sequence, request );
                plasma_dynamic_sync();
            }
    ...
    if (jobvt == PlasmaVec){
        ...
        if( NB < N ){
            ...
            if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
                plasma_cooplap2tile( descVT, VT, NB, NB, LDVT, N, 0, 0, NE, N, sequence, request, plasma_desc_mat_free(&(descVT)) );
            } else {
                plasma_ciplap2tile( descVT, VT, NB, NB, LDVT, N, 0, 0, NE, N, sequence, request);
            }
