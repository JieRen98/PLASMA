                  ...
                  float *C, int LDC)
{
    ...
    if (LDC < max(1, M)) {
        plasma_error("PLASMA_sormlq", "illegal value of LDC");
        return -10;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_sooplap2tile( descA, A, NB, NB, LDA, An, 0, 0, K, An, sequence, &request,
                       ...
                       PLASMA_desc *A, PLASMA_desc *T, PLASMA_desc *C)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_sormlq_Tile_Async(side, trans, A, T, C, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
