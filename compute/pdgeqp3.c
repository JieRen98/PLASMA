...
static const double zone  = (double)  1.;
static const double mzone = (double) -1.;
static const double zzero = (double)  0.;
                           ...
                           PLASMA_sequence *sequence, PLASMA_request *request )
{
    ...
    QUARK_CORE_dgeqp3_init( plasma->quark, &task_flags, A.n, jpvt );

    /* Compute the initial norm of each column of A */
    for( kk = 0; kk < A.nt; kk++ ) {
        kn = BLKN( A, kk );
        /* set norms2[:] = -1. to force dgeqp3_norms to compute all column norms */
        QUARK_CORE_dlaset( plasma->quark, &task_flags,
            PlasmaUpperLower, kn, 1, mzone, mzone, &norms2[kk*A.nb], 1 );
        QUARK_CORE_dgeqp3_norms( plasma->quark, &task_flags,
    ...
    while( j < minmn ) {
        ...
        while( k < jb ) {
            ...
            QUARK_CORE_dgeqp3_pivot( plasma->quark, &task_flags,
            ...
            if ( k > 0 ) {
                ...
                for( ii = jj; ii < A.mt; ++ii ) {
                    ...
                    QUARK_CORE_dgemm_tile( plasma->quark, &task_flags,
            ...
            /* Householder */
            QUARK_CORE_dgeqp3_larfg( plasma->quark, &task_flags,
            ...
            if ( k < nj-1 ) {
                ...
                for( ii = jj; ii < A.mt; ++ii ) {
                    ...
                    for( kk = jj; kk < A.nt; ++kk ) {
                        ...
                        Aik = A(ii,kk);
                        Fk  = &F[(kk-jj)*A.nb];
                        QUARK_CORE_dgemv_tile( plasma->quark, &task_flags,
            ...
            for( kk = jj; kk < A.nt; ++kk ) {
                ...
                Ajk = A(jj,kk);
                Fk  = &F[(kk-jj)*A.nb];
                QUARK_CORE_dgeqp3_update( plasma->quark, &task_flags,
            ...
            /* save beta (from dlarfg) to diagonal */
            QUARK_CORE_dsetvar( plasma->quark, &task_flags,
