 ...
 *
 * @sa PLASMA_dstedc
 * @sa PLASMA_dstedc_Async
 * @sa PLASMA_cstedc
 * @sa PLASMA_dstedc
 * @sa PLASMA_sstedc
 *
 ******************************************************************************/
int PLASMA_dstedc(PLASMA_enum jobz, int n,
                  double *D, double *E,
                  double *Z, int LDZ)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dstedc", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dstedc_Async(jobz, n, D, E, Z, LDZ, sequence, &request);
    plasma_dynamic_sync();
 ...
 *
 *  PLASMA_dstedc_Async - Computes all eigenpairs of a symmetric tridiagonal matrix
 *
 *******************************************************************************
 *
 * @param[in] jobz
 ...
 *******************************************************************************/
#ifdef REAL
int PLASMA_dstedc_Async(PLASMA_enum jobz, int n,
                        double *D, double *E,
                        double *Z, int LDZ,
                        PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dstedc_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_dstedc_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_dstedc_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (jobz != PlasmaNoVec && jobz != PlasmaIvec) {
        plasma_error("PLASMA_dstedc_Async", "illegal value of jobz");
        return -1;
    }

    if (n < 0){
        plasma_error("PLASMA_dstedc_Async", "illegal value of n");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (D == NULL){
        plasma_error("PLASMA_dstedc_Async", "illegal value of D");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (E == NULL){
        plasma_error("PLASMA_dstedc_Async", "illegal value of E");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (Z == NULL && (jobz == PlasmaVec) ){
        plasma_error("PLASMA_dstedc_Async", "illegal value of Z");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (LDZ < max(1, n)){
        plasma_error("PLASMA_dstedc_Async", "illegal value of LDZ");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ((n < SMLSIZ) || (jobz == PlasmaNoVec)) {
        if (n < SMLSIZ) LAPACKE_dlaset_work( LAPACK_COL_MAJOR, lapack_const(PlasmaUpperLower), n, n, 0.0, 1.0, Z, LDZ);
        info = LAPACKE_dstedc( LAPACK_COL_MAJOR, lapack_const(jobz),
                               n, D, E, Z, LDZ);
        if (info != 0){
            plasma_error("PLASMA_dstedc_Async", "LAPACKE dstedc failed");
            return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
        }
        else{
    ...
    while (i<n-1){
        ...
        if ((fabs(E[i]) <= tiny) || (i==n-2)){
            ...
            if (size == 1){
                ...
                plasma_dynamic_call_21( plasma_pdlaed0,
    ...
    if (act_perm == 1){
        plasma_dynamic_call_7(plasma_pdswaps,
                              int, n,
                              int*, IWORK,
                              double*, Z,
                              int, LDZ,
    ...
    plasma_printtime("  Finish dstedc with nsub_dlaed0 %5d     nsub_smlsiz %5d   nsub1 %5d timing= %lf \n",nsubdlaed0, nsubsml, nsub1, timestedc);
