...
void plasma_pcgetrf_rectil_rl_quark(PLASMA_desc A, int *IPIV, PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma_profile_by_function( &task_flagsP, GETRF );
    plasma_profile_by_function( &task_flagsU, GETRF );
    ...
    data = CORE_cgetrf_rectil_init(panel_thread_count);
    ...
    for (k = 0; k < min(A.mt, A.nt); k++)
    {
        ...
        QUARK_CORE_cgetrf_rectil(
            ...
            data, plasma_desc_submatrix(A, tempk, k*A.nb, tempm, tempkn),
    ...
    QUARK_CORE_free(
        plasma->quark, &task_flagsP, data, 1);
}
...
void plasma_pcgetrf_tile_ll_quark(PLASMA_desc A, int *IPIV, PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    for (n = 0; n < A.nt; n++)
    {
        ...
        if ( n < A.mt ) {
            ...
            QUARK_CORE_cgetrf_rectil(
                plasma->quark, &task_flagsP,
                data, plasma_desc_submatrix(A, k*A.mb, k*A.nb, tempm, tempkn), A(k, k), A.mb*A.nb,
