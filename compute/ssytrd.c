 ...
 *
 * @param[out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_ssytrd
                 ...
                 float *Q, int LDQ)
{
    ...
    PLASMA_ssytrd_Tile_Async(jobz, uplo, &descA, D, E, descT, Q, LDQ, sequence, &request);
                       ...
                       PLASMA_desc *T, float *Q, int LDQ)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_ssytrd_Tile_Async(jobz, uplo, A, D, E, T, Q, LDQ, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
                            ...
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_ssytrd_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.m != descA.n) {
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_ssytrd", "plasma_shared_alloc() failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_ssytrd","pssytrd_he2hb+pscopy");
        return status;
    }
    /*=======================================
