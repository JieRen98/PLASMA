 ...
 * @ingroup float
 *
 *  PLASMA_sgetrf_tntpiv - Computes an LU factorization of a general M-by-N
 ...
 *
 ******************************************************************************/
int PLASMA_sgetrf_tntpiv(int M, int N,
                         float *A, int LDA,
                         int *IPIV)
{
    int NB;
    ...
    PLASMA_Alloc_Workspace_sgetrf_tntpiv_Tile(&descA, &W, &Wpivot);
    PLASMA_sgetrf_tntpiv_Tile_Async(&descA, IPIV, &W, Wpivot, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_sgetrf_tntpiv_Tile - Computes the tile LU factorization of a matrix.
 *  Tile equivalent of PLASMA_sgetrf_tntpiv().
...
int PLASMA_sgetrf_tntpiv_Tile(PLASMA_desc *A, int *IPIV)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_tntpiv_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_Alloc_Workspace_sgetrf_tntpiv_Tile(A, &W, &Wpivot);
    PLASMA_sgetrf_tntpiv_Tile_Async(A, IPIV, &W, Wpivot, sequence, &request);
    plasma_dynamic_sync();
    plasma_desc_mat_free(&W);
    free(Wpivot);
    status = sequence->status;
 ...
 *
 *  PLASMA_sgetrf_tntpiv_Tile_Async - Computes the tile LU factorization of a
 *  matrix.  Non-blocking equivalent of PLASMA_sgetrf_tntpiv_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_sgetrf_tntpiv_Tile_Async(PLASMA_desc *A, int *IPIV,
                                    ...
                                    PLASMA_request *request)
{
    ...
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_tntpiv_Tile", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_tntpiv_Tile", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (PLASMA_TNT_MODE != PLASMA_TOURNAMENT_LU) {
        plasma_fatal_error("PLASMA_sgetrf_tntpiv_Tile", "Only PLASMA_TOURNAMENT_LU supported");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sgetrf_tntpiv_Tile", "invalid first descriptor");
    ...
    if (plasma_desc_check(W) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sgetrf_tntpiv_Tile_Async", "invalid W descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descW = *W;
    }
    ...
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_sgetrf_tntpiv_Tile", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    plasma_dynamic_call_6(
        plasma_psgetrf_tntpiv,
        PLASMA_desc,      descA,
        int*,             IPIV,
        PLASMA_desc,      descW,
        int*,             Wpivot,
...
int PLASMA_Alloc_Workspace_sgetrf_tntpiv_Tile(PLASMA_desc *A,
                                              PLASMA_desc *W,
                                              int **Wpivot)
{
    plasma_context_t *plasma;
    ...
    plasma_sdesc_alloc( *W, rndsize*A->mb, A->nb, wm, nblevel*A->nb,
