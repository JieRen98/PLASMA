 ...
 * @ingroup double
 *
 *  PLASMA_dpltmg - Generate a test matrix by tiles.
 *
 *******************************************************************************
 *
 * @param[in] mtxtype
 ...
 *
 ******************************************************************************/
int PLASMA_dpltmg( PLASMA_enum mtxtype, int M, int N,
                    double *A, int LDA,
                    unsigned long long int seed )
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dpltmg", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_dpltmg", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_dpltmg", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_dpltmg", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dpltmg", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_dpltmg_Tile_Async( mtxtype, &descA, seed, sequence, &request );
 ...
 * @ingroup double_Tile
 *
 *  PLASMA_dpltmg_Tile - Generate a random matrix by tiles.
 *  Tile equivalent of PLASMA_dpltmg().
 ...
 *
 ******************************************************************************/
int PLASMA_dpltmg_Tile( PLASMA_enum mtxtype, PLASMA_desc *A,
                         unsigned long long int seed )
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dpltmg_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dpltmg_Tile_Async( mtxtype, A, seed, sequence, &request );
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_dpltmg_Tile_Async - Generate a random matrix by tiles.
 *  Non-blocking equivalent of PLASMA_dpltmg_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_dpltmg_Tile_Async( PLASMA_enum mtxtype, PLASMA_desc *A,
                               unsigned long long int seed,
                               ...
                               PLASMA_request  *request)
{
    ...
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_dpltmg_Tile", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_dpltmg_Tile", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dpltmg_Tile", "invalid descriptor");
    ...
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_dpltmg_Tile", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    case PlasmaMatrixCircul:
        plasma_dynamic_call_4(plasma_pdpltmg_circul,
            PLASMA_desc,            descA,
            unsigned long long int, seed,
            PLASMA_sequence*,       sequence,
            PLASMA_request*,        request);
    ...
    case PlasmaMatrixChebvand:
        plasma_dynamic_call_3(plasma_pdpltmg_chebvand,
            PLASMA_desc,            descA,
            PLASMA_sequence*,       sequence,
            PLASMA_request*,        request);
        break;
    ...
    case PlasmaMatrixCondex:
        plasma_dpltmg_condex( descA, sequence, request );
        break;

    case PlasmaMatrixFiedler:
        if (descA.m != descA.n) {
            plasma_error("PLASMA_dpltmg_Tile_Async", "Only square fiedler matrices can be generated");
            return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
        }
        plasma_dynamic_call_4(plasma_pdpltmg_fiedler,
            PLASMA_desc,            descA,
            unsigned long long int, seed,
            PLASMA_sequence*,       sequence,
            PLASMA_request*,        request);
    ...
    case PlasmaMatrixHankel:
        plasma_dynamic_call_4(plasma_pdpltmg_hankel,
            PLASMA_desc,            descA,
            unsigned long long int, seed,
            PLASMA_sequence*,       sequence,
            PLASMA_request*,        request);
    ...
    case PlasmaMatrixHouse:
        plasma_dpltmg_house( descA, seed, sequence, request );
        break;

    case PlasmaMatrixToeppd:
        plasma_dynamic_call_4(plasma_pdpltmg_toeppd,
            PLASMA_desc,            descA,
            unsigned long long int, seed,
            PLASMA_sequence*,       sequence,
            PLASMA_request*,        request);
    ...
    default:
        plasma_error("PLASMA_dpltmg_Tile", "Illegal value of mtxtype");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
