 ...
 *
 ******************************************************************************/
int PLASMA_dpotrf_gpu(PLASMA_enum uplo, int N,
                  double *A, int LDA)
{
    int NB;
    int status;
    ...
    PLASMA_dpotrf_gpu_Tile_Async(uplo, &descA, sequence, &request);
...
int PLASMA_dpotrf_gpu_Tile(PLASMA_enum uplo, PLASMA_desc *A)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dpotrf_gpu_Tile_Async(uplo, A, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 ******************************************************************************/
int PLASMA_dpotrf_gpu_Tile_Async(PLASMA_enum uplo, PLASMA_desc *A,
...
/*
    if (max(N, 0) == 0)
        return PLASMA_SUCCESS;
*/
    plasma_parallel_call_4(plasma_pdpotrf_gpu,
        PLASMA_enum, uplo,
        PLASMA_desc, descA,
