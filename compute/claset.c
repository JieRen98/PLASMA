 ...
 *
 *  PLASMA_claset sets all or part of a two-dimensional matrix A.
 *
 *******************************************************************************
 *
 * @param[in] uplo
 ...
 *
 ******************************************************************************/
int PLASMA_claset(PLASMA_enum uplo, int M, int N,
                  ...
                  PLASMA_Complex32_t *A, int LDA)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_claset", "PLASMA not initialized");
    ...
    if ( (uplo != PlasmaUpperLower) &&
         (uplo != PlasmaUpper) &&
         (uplo != PlasmaLower) ) {
        plasma_error("PLASMA_claset", "illegal value of uplo");
        return -1;
    }
    if (M < 0) {
        plasma_error("PLASMA_claset", "illegal value of M");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_claset", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_claset", "illegal value of LDA");
        return -5;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_claset", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_claset_Tile_Async(uplo, alpha, beta, &descA, sequence, &request);
 ...
 *
 *  PLASMA_claset_Tile - Tile equivalent of PLASMA_claset().
 ...
 *
 *  PLASMA_claset_Tile_Async - Non-blocking equivalent of PLASMA_claset_Tile().
