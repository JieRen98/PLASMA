 ...
 *
 *  PLASMA_cstedc - Computes all eigenpairs of a symmetric tridiagonal matrix
 *
 *******************************************************************************
 *
 * @param[in] jobz
 ...
 *
 * @sa PLASMA_cstedc
 * @sa PLASMA_cstedc_Async
 * @sa PLASMA_cstedc
 * @sa PLASMA_sstedc
 * @sa PLASMA_sstedc
 *
 ******************************************************************************/
int PLASMA_cstedc(PLASMA_enum jobz, int n,
                  float *D, float *E,
                  PLASMA_Complex32_t *Z, int LDZ)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cstedc", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_cstedc_Async(jobz, n, D, E, Z, LDZ, sequence, &request);
    plasma_dynamic_sync();
 ...
 *
 *  PLASMA_cstedc_Async - Computes all eigenpairs of a symmetric tridiagonal matrix
 *
 *******************************************************************************
 *
 * @param[in] jobz
 ...
 *******************************************************************************/
#ifdef REAL
int PLASMA_cstedc_Async(PLASMA_enum jobz, int n,
                        float *D, float *E,
                        PLASMA_Complex32_t *Z, int LDZ,
                        PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cstedc_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_cstedc_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_cstedc_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (jobz != PlasmaNoVec && jobz != PlasmaIvec) {
        plasma_error("PLASMA_cstedc_Async", "illegal value of jobz");
        return -1;
    }

    if (n < 0){
        plasma_error("PLASMA_cstedc_Async", "illegal value of n");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (D == NULL){
        plasma_error("PLASMA_cstedc_Async", "illegal value of D");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (E == NULL){
        plasma_error("PLASMA_cstedc_Async", "illegal value of E");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (Z == NULL && (jobz == PlasmaVec) ){
        plasma_error("PLASMA_cstedc_Async", "illegal value of Z");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    if (LDZ < max(1, n)){
        plasma_error("PLASMA_cstedc_Async", "illegal value of LDZ");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ((n < SMLSIZ) || (jobz == PlasmaNoVec)) {
        if (n < SMLSIZ) LAPACKE_claset_work( LAPACK_COL_MAJOR, lapack_const(PlasmaUpperLower), n, n, 0.0, 1.0, Z, LDZ);
        info = LAPACKE_cstedc( LAPACK_COL_MAJOR, lapack_const(jobz),
                               n, D, E, Z, LDZ);
        if (info != 0){
            plasma_error("PLASMA_cstedc_Async", "LAPACKE cstedc failed");
            return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
        }
        else{
    ...
    while (i<n-1){
        ...
        if ((fabs(E[i]) <= tiny) || (i==n-2)){
            ...
            if (size == 1){
                ...
                plasma_dynamic_call_21( plasma_pclaed0,
    ...
    if (act_perm == 1){
        plasma_dynamic_call_7(plasma_pcswaps,
                              int, n,
                              int*, IWORK,
                              float*, Z,
                              int, LDZ,
