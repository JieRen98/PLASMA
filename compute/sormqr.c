 ...
 * @ingroup float
 *
 *  PLASMA_sormqr - Overwrites the general complex M-by-N matrix C with
 *
 *                  SIDE = 'L'     SIDE = 'R'
 *  TRANS = 'N':      Q * C          C * Q
 *  TRANS = 'C':      Q**T * C       C * Q**T
                  ...
                  float *C, int LDC)
{
    ...
    if (LDC < max(1, M)) {
        plasma_error("PLASMA_sormqr", "illegal value of LDC");
        return -10;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_sooplap2tile( descA, A, NB, NB, LDA, K, 0, 0, Am, K, sequence, &request,
 ...
 * @param[in] T
 *          Auxiliary factorization data, computed by PLASMA_sgeqrf.
 *          Can be obtained with PLASMA_Alloc_Workspace_sgeqrf
 *
 * @param[in,out] C
 *          On entry, the M-by-N matrix C.
 *          On exit, C is overwritten by Q*C or Q**T*C.
                       ...
                       PLASMA_desc *A, PLASMA_desc *T, PLASMA_desc *C)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_sormqr_Tile_Async(side, trans, A, T, C, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
