 ...
 **/
void plasma_pcpltmg_toeppd_quark( PLASMA_desc A, unsigned long long int seed,
                                  PLASMA_sequence *sequence, PLASMA_request *request )
{
    ...
    work = (PLASMA_Complex32_t**) malloc ( A.mt * sizeof( PLASMA_Complex32_t* ) );
    ...
    for (k = 0; k < A.mt; k++) {
        ...
        work[k] = (PLASMA_Complex32_t*)plasma_shared_alloc(plasma, 2*tempkm, PlasmaComplexFloat);

        QUARK_CORE_cpltmg_toeppd1(
    ...
    for (k = 0; k < A.mt; k++) {
        ...
        QUARK_CORE_free(plasma->quark, &task_flags,
                        work[k], (2*tempkm)*sizeof(PLASMA_Complex32_t));
    }
