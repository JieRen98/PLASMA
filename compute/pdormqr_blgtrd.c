...
void plasma_pdormqr_blgtrd(plasma_context_t *plasma)
{
    ...
    if ( NB == 1 ) {
        ...
        if(WANTZ==1){
            double zone = 1.0;
            memset(E, 0, LDE*N*sizeof(double));
            for(sw=0; sw<NE; sw++)
                E[sw+sw*LDE] = zone;
        }
        if(standalonework==0){
                ...
                if( my_core_id==coreid ){
                    if( side==PlasmaLeft ){
                        for (mycol =1; mycol<NE; mycol++){
                            cblas_dscal(corelen, TAU(mycol), E(mycol, corest), LDE);
                        }
                    }else{
                        double ztmp;
                        ...
                        for (mycol = corest; mycol<corest+corelen; mycol++){
                            ztmp = (*TAU(mycol));
                            cblas_dscal(N, &ztmp, E(0,mycol), 1);
                        }
                    }
                }
            }
    ...
    WORK  = (double*) plasma_private_alloc(plasma, LWORK, PlasmaRealDouble);
