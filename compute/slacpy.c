 ...
 * @ingroup float
 *
 *  PLASMA_slacpy copies all or part of a two-dimensional matrix A to another
 *  matrix B
 *
 *******************************************************************************
 *
 ...
 *
 ******************************************************************************/
int PLASMA_slacpy(PLASMA_enum uplo, int M, int N,
                  float *A, int LDA,
                  float *B, int LDB)
{
    int NB;
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_slacpy", "PLASMA not initialized");
    ...
    if ( (uplo != PlasmaUpperLower) &&
         (uplo != PlasmaUpper) &&
         (uplo != PlasmaLower) ) {
        plasma_error("PLASMA_slacpy", "illegal value of uplo");
        return -1;
    }
    if (M < 0) {
        plasma_error("PLASMA_slacpy", "illegal value of M");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_slacpy", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_slacpy", "illegal value of LDA");
        return -5;
    }
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_slacpy", "illegal value of LDB");
        return -7;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_slacpy", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        ...
        plasma_siplap2tile(  descA, A, NB, NB, LDA, N, 0, 0, M, N,
                            sequence, &request);
        plasma_siplap2tile(  descB, B, NB, NB, LDA, N, 0, 0, M, N,
                            sequence, &request);
    }

    /* Call the tile interface */
    PLASMA_slacpy_Tile_Async(uplo, &descA, &descB, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_slacpy_Tile - Tile equivalent of PLASMA_slacpy().
 ...
 *
 *  PLASMA_slacpy_Tile_Async - Non-blocking equivalent of PLASMA_slacpy_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_slacpy_Tile_Async(PLASMA_enum uplo, PLASMA_desc *A, PLASMA_desc *B,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_slacpy_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_slacpy_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    ...
    plasma_parallel_call_5(plasma_pslacpy,
        PLASMA_enum, uplo,
        PLASMA_desc, descA,
        PLASMA_desc, descB,
