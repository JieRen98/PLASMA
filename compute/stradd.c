 ...
 * @ingroup float
 *
 *  PLASMA_stradd - Performs a matrix addition similarly to the pstradd()
 ...
 *
 ******************************************************************************/
int PLASMA_stradd(PLASMA_enum uplo, PLASMA_enum trans, int M, int N,
                  ...
                  float beta,  float *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_stradd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((uplo != PlasmaUpperLower) && (uplo != PlasmaUpper) && (uplo != PlasmaLower)) {
        plasma_error("PLASMA_stradd", "illegal value of uplo");
        return -1;
    }
    if ((trans != PlasmaNoTrans) && (trans != PlasmaTrans) && (trans != PlasmaTrans)) {
        plasma_error("PLASMA_stradd", "illegal value of trans");
        return -2;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_stradd", "illegal value of M");
        return -3;
    }
    if (N < 0) {
        plasma_error("PLASMA_stradd", "illegal value of N");
        return -4;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_stradd", "illegal value of LDA");
        return -7;
    }
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_stradd", "illegal value of LDB");
        return -10;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_stradd", "plasma_tune() failed");
        return status;
    }
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_stradd_Tile - Performs a matrix addition similarly to the pstradd()
 *  function from the PBLAS library.
 *  Tile equivalent of PLASMA_stradd().
 ...
 *
 ******************************************************************************/
int PLASMA_stradd_Tile(PLASMA_enum uplo, PLASMA_enum trans,
                       float alpha, PLASMA_desc *A,
                       float beta,  PLASMA_desc *B)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    PLASMA_request request = PLASMA_REQUEST_INITIALIZER;
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_stradd_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_stradd_Tile_Async(uplo, trans, alpha, A, beta, B, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_stradd_Tile_Async - Performs a matrix addition similarly to the
 *  pstradd() function from the PBLAS library.
 *  Non-blocking equivalent of PLASMA_stradd_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_stradd_Tile_Async(PLASMA_enum uplo, PLASMA_enum trans,
                             float alpha, PLASMA_desc *A,
                             float beta,  PLASMA_desc *B,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_stradd_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_stradd_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_stradd_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_stradd_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_stradd_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    /* Check input arguments */
    if ((trans != PlasmaNoTrans) && (trans != PlasmaTrans) && (trans != PlasmaTrans)) {
        plasma_error("PLASMA_stradd_Tile_Async", "illegal value of trans");
    ...
    if ( (Amb != descB.mb) || (Anb != descB.nb) ) {
        plasma_error("PLASMA_stradd_Tile_Async", "tile sizes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Am != descB.m) || (An != descB.n) ) {
        plasma_error("PLASMA_stradd_Tile_Async", "sizes of matrices have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Ai%Amb != descB.i%descB.mb) ||
         (Aj%Anb != descB.j%descB.nb) )
    {
        plasma_error("PLASMA_stradd_Tile_Async", "start indexes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    plasma_parallel_call_8(plasma_pstradd,
        PLASMA_enum, uplo,
        PLASMA_enum, trans,
        float, alpha,
        PLASMA_desc, descA,
