                 ...
                 PLASMA_Complex32_t *Q, int LDQ)
{
    ...
    PLASMA_chegv_Tile_Async(itype, jobz, uplo,
                            &descA, &descB, W,
                            descT, &descQ,
                            sequence, &request);
                            ...
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma_desc_check(T) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_chegv_Tile_Async", "invalid T descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (plasma_desc_check(Q) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_chegv_Tile_Async", "invalid Q descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descQ = *Q;
    }
    ...
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_chegv_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
     ...
     * Call Eigensolver
     */
    status = PLASMA_cheev_Tile_Async( jobz, uplo,
                                      A, W, T,
