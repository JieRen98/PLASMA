 ...
 * @ingroup double
 *
 *  PLASMA_dsyevd - Computes all eigenvalues and, optionally,
 ...
 *
 * @param[in, out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_dsyevd
 ...
 *
 ******************************************************************************/
int PLASMA_dsyevd(PLASMA_enum jobz, PLASMA_enum uplo, int N,
                 double *A, int LDA,
                 double *W,
                 PLASMA_desc *descT,
                 double *Q, int LDQ)
{
    ...
    if (plasma == NULL) {
        plasma_error("PLASMA_dsyevd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_dsyevd", "illegal value of jobz");
        return -1;
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_dsyevd", "illegal value of uplo");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_dsyevd", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_dsyevd", "illegal value of LDA");
        return -5;
    }
    if (LDQ < max(1, N)) {
        plasma_error("PLASMA_dsyevd", "illegal value of LDQ");
        return -9;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsyevd", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_dsyevd_Tile_Async(jobz, uplo, &descA, W, descT, Q, LDQ, sequence, &request);
 ...
 * @ingroup double_Tile
 *
 *  PLASMA_dsyevd_Tile - Computes all eigenvalues and, optionally, eigenvectors of a
 ...
 *
 ******************************************************************************/
int PLASMA_dsyevd_Tile(PLASMA_enum jobz, PLASMA_enum uplo,
                      ...
                      PLASMA_desc *T, double *Q, int LDQ)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dsyevd_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dsyevd_Tile_Async(jobz, uplo, A, W, T, Q, LDQ, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_dsyevd_Tile_Async - Computes all eigenvalues and,
 ...
 *
 ******************************************************************************/
int PLASMA_dsyevd_Tile_Async(PLASMA_enum jobz, PLASMA_enum uplo,
                            PLASMA_desc *A,
                            double *W,
                            PLASMA_desc *T,
                            double *Q, int LDQ,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dsyevd_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_dsyevd_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_dsyevd_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "invalid descriptor");
    ...
    /* Check input arguments */
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "illegal value of jobz");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.m != descA.n) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "matrix need to be square");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "plasma_shared_alloc(AB) failed");
        plasma_shared_free(plasma, AB);
        return PLASMA_ERR_OUT_OF_RESOURCES;
    }
    E = (double *)plasma_shared_alloc(plasma, N, PlasmaRealDouble);
    if (E == NULL) {
        plasma_error("PLASMA_dsyevd_Tile_Async", "plasma_shared_alloc(E) failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsyevd","pdsytrd_he2hb+pdcopy");
        return status;
    }
    /*=======================================
    ...
    if( jobz == PlasmaVec ) {
        ...
        if ( (TAU2 == NULL) || (V2 == NULL) || (T2 == NULL) ) {
            plasma_error("PLASMA_dsyevd", "plasma_shared_alloc() failed");
    ...
    plasma_static_call_13(plasma_pdsytrd_hb2st_v1,
        PLASMA_enum,         blguplo,
        int,                 N,
        int,                 NB,
        int,                 Vblksiz,
    ...
    /*
    status = LAPACKE_dstevd( LAPACK_COL_MAJOR, lapack_const(jobz), N, W, E, Q, LDQ );
    */
    if(jobz == PlasmaNoVec){
        status = LAPACKE_dstedc( LAPACK_COL_MAJOR, lapack_const(PlasmaNoVec),
                        N, W, E, Q, LDQ );
    } else {
        status = LAPACKE_dstedc( LAPACK_COL_MAJOR, lapack_const(PlasmaIvec),
                        N, W, E, Q, LDQ );
    }
...
#else
    {
        ...
        status = PLASMA_dstedc_Async(jobz_stedc, N, W, E, Q, LDQ, sequence, request);
    }
#endif
