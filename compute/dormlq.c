 ...
 * @ingroup double
 *
 *  PLASMA_dormlq - Overwrites the general complex M-by-N matrix C with
 *
 *                  SIDE = 'L'     SIDE = 'R'
 *  TRANS = 'N':      Q * C          C * Q
 *  TRANS = 'C':      Q**T * C       C * Q**T
                  ...
                  double *C, int LDC)
{
    ...
    if (LDC < max(1, M)) {
        plasma_error("PLASMA_dormlq", "illegal value of LDC");
        return -10;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_dooplap2tile( descA, A, NB, NB, LDA, An, 0, 0, K, An, sequence, &request,
                       ...
                       PLASMA_desc *A, PLASMA_desc *T, PLASMA_desc *C)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dormlq_Tile_Async(side, trans, A, T, C, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
