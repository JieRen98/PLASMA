 ...
 * @ingroup double
 *
 *  PLASMA_dsyevr - Computes all eigenvalues and, optionally,
 ...
 *
 * @param[in, out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_dsyevr
 ...
 *
 ******************************************************************************/
int PLASMA_dsyevr(PLASMA_enum jobz, PLASMA_enum range, PLASMA_enum uplo,
                 int N,
                 double *A, int LDA,
                 double vl, double vu, int il, int iu,
                 double abstol, int *nbcomputedeig,
                 double *W,
                 PLASMA_desc *descT,
                 double *Q, int LDQ)
{
    ...
    if (plasma == NULL) {
        plasma_error("PLASMA_dsyevr", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_dsyevr", "illegal value of jobz");
        return -1;
    }
    ...
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_dsyevr", "illegal value of uplo");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_dsyevr", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_dsyevr", "illegal value of LDA");
        return -5;
    }
    if (LDQ < max(1, N)) {
        plasma_error("PLASMA_dsyevr", "illegal value of LDQ");
        return -9;
    }
    ...
    status = plasma_tune(PLASMA_FUNC_DSYEVD, N, N, 0);
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsyevr", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_dsyevr_Tile_Async(jobz, range, uplo, &descA, vl, vu, il, iu, abstol, nbcomputedeig, W, descT, Q, LDQ, sequence, &request);
 ...
 * @ingroup double_Tile
 *
 *  PLASMA_dsyevr_Tile - Computes all eigenvalues and, optionally, eigenvectors of a
 ...
 *
 ******************************************************************************/
int PLASMA_dsyevr_Tile(PLASMA_enum jobz, PLASMA_enum range, PLASMA_enum uplo,
                       PLASMA_desc *A,
                       double vl, double vu, int il, int iu,
                       double abstol, int *nbcomputedeig,
                       double *W, PLASMA_desc *T,
                       double *Q, int LDQ)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dsyevr_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dsyevr_Tile_Async(jobz, range, uplo, A, vl, vu, il, iu, abstol, nbcomputedeig, W, T, Q, LDQ, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_dsyevr_Tile_Async - Computes all eigenvalues and,
 ...
 *
 ******************************************************************************/
int PLASMA_dsyevr_Tile_Async(PLASMA_enum jobz, PLASMA_enum range, PLASMA_enum uplo,
                             PLASMA_desc *A,
                             double vl, double vu, int il, int iu,
                             double abstol, int *nbcomputedeig,
                             double *W,
                             ...
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dsyevr_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_dsyevr_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_dsyevr_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "invalid descriptor");
    ...
    /* Check input arguments */
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "illegal value of jobz");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.m != descA.n) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "matrix need to be square");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "plasma_shared_alloc(AB) failed");
        plasma_shared_free(plasma, AB);
        return PLASMA_ERR_OUT_OF_RESOURCES;
    }
    D = (double *)plasma_shared_alloc(plasma, N, PlasmaRealDouble);
    E = (double *)plasma_shared_alloc(plasma, N, PlasmaRealDouble);
    if ((E == NULL) || (D == NULL) ) {
        plasma_error("PLASMA_dsyevr_Tile_Async", "plasma_shared_alloc(E) failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsyevr","pdsytrd_he2hb+pdcopy");
        return status;
    }
    /*=======================================
    ...
    if( jobz == PlasmaVec ) {
       ...
       isuppz  = (int *) plasma_shared_alloc(plasma, 2*N, PlasmaInteger);
        if ( (TAU2 == NULL) || (V2 == NULL) || (T2 == NULL) || (isuppz == NULL)) {
            plasma_error("PLASMA_dsyevr", "plasma_shared_alloc() failed");
    ...
    */
    ...
    status = LAPACKE_dstemr( LAPACK_COL_MAJOR, lapack_const(jobz), lapack_const(range),
                             N, D, E, vl, vu, il, iu, nbcomputedeig,
                             W, Q, LDQ, NE, isuppz, &tryrac);
    if(status != 0){
        plasma_error("PLASMA_dstedc","ZSTEDC");
        return status;
    }
    ...
    if (jobz == PlasmaVec){
        ...
        if( NB < N ){
            ...
            if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
                plasma_dooplap2tile( descQ, Q, NB, NB, LDQ, NE, 0, 0, N, NE, sequence, request, plasma_desc_mat_free(&(descQ)) );
            }else {
                plasma_diplap2tile( descQ, Q, NB, NB, LDQ, NE, 0, 0, N, NE, sequence, request );
            }
            ...
            if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
                plasma_dooptile2lap( descQ, Q, NB, NB, LDQ, NE, sequence, request );
                plasma_dynamic_sync();
                plasma_desc_mat_free(&descQ);
            } else {
                plasma_diptile2lap( descQ, Q, NB, NB, LDQ, NE, sequence, request );
                plasma_dynamic_sync();
            }
