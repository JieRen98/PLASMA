 ...
 *******************************************************************************
 *
 * @sa PLASMA_ctrsmrv_Tile
 * @sa PLASMA_ctrsmrv_Tile_Async
 * @sa PLASMA_ctrsmrv
 * @sa PLASMA_dtrsmrv
 * @sa PLASMA_strsmrv
 *
 ******************************************************************************/
int PLASMA_ctrsmrv(PLASMA_enum side, PLASMA_enum uplo,
                 ...
                 PLASMA_Complex32_t *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_ctrsmrv", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (side != PlasmaLeft && side != PlasmaRight) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of side");
        return -1;
    }
    if (uplo != PlasmaUpper && uplo != PlasmaLower) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of uplo");
        return -2;
    }
    if (transA != PlasmaConjTrans && transA != PlasmaNoTrans && transA != PlasmaTrans ) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of transA");
        return -3;
    }
    if (diag != PlasmaUnit && diag != PlasmaNonUnit) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of diag");
        return -4;
    }
    if (N < 0) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of N");
        return -5;
    }
    if (NRHS < 0) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of NRHS");
        return -6;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, N)) {
        plasma_error("PLASMA_ctrsmrv", "illegal value of LDB");
        return -10;
    }
    /* Quick return */
    if (min(N, NRHS) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_ctrsmrv", "plasma_tune() failed");
        return status;
    }
 ...
 *
 *  PLASMA_ctrsmrv_Tile - Computes triangular solve.
 *  Tile equivalent of PLASMA_ctrsmrv().
 ...
 * @sa PLASMA_ctrsmrv_Tile_Async
 * @sa PLASMA_ctrsmrv_Tile
 * @sa PLASMA_dtrsmrv_Tile
 * @sa PLASMA_strsmrv_Tile
 *
 ******************************************************************************/
int PLASMA_ctrsmrv_Tile(PLASMA_enum side, PLASMA_enum uplo,
                      PLASMA_enum transA, PLASMA_enum diag,
 ...
 *
 *  PLASMA_ctrsmrv_Tile_Async - Computes triangular solve.
 *  Non-blocking equivalent of PLASMA_ctrsmrv_Tile().
 ...
 * @sa PLASMA_ctrsmrv_Tile
 * @sa PLASMA_ctrsmrv_Tile_Async
 * @sa PLASMA_dtrsmrv_Tile_Async
 * @sa PLASMA_strsmrv_Tile_Async
 *
 ******************************************************************************/
int PLASMA_ctrsmrv_Tile_Async(PLASMA_enum side, PLASMA_enum uplo,
                            PLASMA_enum transA, PLASMA_enum diag,
                            PLASMA_Complex32_t alpha, PLASMA_desc *A, PLASMA_desc *B,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_ctrsmrv_Tile", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_ctrsmrv_Tile", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_ctrsmrv_Tile", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_ctrsmrv_Tile", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    ...
    if (descA.nb != descA.mb || descB.nb != descB.mb) {
        plasma_error("PLASMA_ctrsmrv_Tile", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (side != PlasmaLeft && side != PlasmaRight) {
        plasma_error("PLASMA_ctrsmrv_Tile", "illegal value of side");
        return plasma_request_fail(sequence, request, -1);
    }
    if (uplo != PlasmaUpper && uplo != PlasmaLower) {
        plasma_error("PLASMA_ctrsmrv_Tile", "illegal value of uplo");
        return plasma_request_fail(sequence, request, -2);
    }
    if (transA != PlasmaConjTrans && transA != PlasmaNoTrans && transA != PlasmaTrans) {
        plasma_error("PLASMA_ctrsmrv_Tile", "illegal value of transA");
        return plasma_request_fail(sequence, request, -3);
    }
    if (diag != PlasmaUnit && diag != PlasmaNonUnit) {
        plasma_error("PLASMA_ctrsmrv_Tile", "illegal value of diag");
        return plasma_request_fail(sequence, request, -4);
    }

    if ( side != PlasmaRight ) {
        plasma_error("PLASMA_ctrsmrv_Tile", "PlasmaLeft is not supported");
        return plasma_request_fail(sequence, request, -1);
    }
    if ( uplo != PlasmaLower ) {
        plasma_error("PLASMA_ctrsmrv_Tile", "PlasmaUpper is not supported");
        return plasma_request_fail(sequence, request, -2);
    }
    if ( transA != PlasmaNoTrans ) {
        plasma_error("PLASMA_ctrsmrv_Tile", "Only PlasmaNoTrans is supported");
        return plasma_request_fail(sequence, request, -3);
    }
    if ( diag != PlasmaUnit ) {
        plasma_error("PLASMA_ctrsmrv_Tile", "PlasmaNonUnit is not supported");
        return plasma_request_fail(sequence, request, -4);
    }
