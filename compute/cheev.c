                 ...
                 PLASMA_Complex32_t *Q, int LDQ)
{
    ...
    PLASMA_cheev_Tile_Async(jobz, uplo, &descA, W, descT, Q, LDQ, sequence, &request);
                      ...
                      PLASMA_desc *T, PLASMA_Complex32_t *Q, int LDQ)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_cheev_Tile_Async(jobz, uplo, A, W, T, Q, LDQ, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_cheev_Tile_Async - Computes all eigenvalues and,
                            ...
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_cheev_Tile_Async", "plasma_shared_alloc(AB) failed");
        plasma_shared_free(plasma, AB);
        return PLASMA_ERR_OUT_OF_RESOURCES;
    }
    E = (float *)plasma_shared_alloc(plasma, N, PlasmaRealDouble);
    if (E == NULL) {
        plasma_error("PLASMA_cheev_Tile_Async", "plasma_shared_alloc(E) failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cheev","pchetrd_he2hb+pccopy");
        return status;
    }
    /*=======================================
    ...
    if( jobz == PlasmaVec ) {
        ...
        if ( (TAU2 == NULL) || (V2 == NULL) || (T2 == NULL) ) {
            plasma_error("PLASMA_cheev", "plasma_shared_alloc() failed");
    ...
    if (jobz == PlasmaVec){
        ...
        memset(Q,   0, N*LDQ*sizeof(PLASMA_Complex32_t));
        for(i=0; i<N; i++){
            Q[i+i*LDQ] = 1.0;
        }
        /* compute T2 */
    ...
    status = LAPACKE_csteqr( LAPACK_COL_MAJOR, lapack_const(jobz),
                    N, W, E, Q, LDQ );
    if(status != 0){
