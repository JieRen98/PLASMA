 ...
 *
 */
void plasma_pspltmg_hankel_quark( PLASMA_desc A, unsigned long long int seed,
                                 PLASMA_sequence *sequence, PLASMA_request *request )
{
    ...
    V = (float**) malloc ( sizeMT * sizeof( float* ) );
    for (k = 0; k < sizeMT; k++) {
        tempk0 = k * A.mb;

        /* Allocate temporary vector and initialize it randomly */
        V[k] = (float*)plasma_shared_alloc(plasma, A.mb, PlasmaRealFloat);
