 ...
 * @ingroup double
 *
 * PLASMA_dgetri - Computes the inverse of a matrix using the LU factorization
 ...
 * @ingroup double_Tile
 *
 *  PLASMA_dgetri_Tile - Computes the inverse of a matrix using the LU factorization
 *  computed by PLASMA_dgetrf.
 *  This method inverts U and then computes inv(A) by solving the system
 *  inv(A)*L = inv(U) for inv(A).
 *  Tile equivalent of PLASMA_dgetri().
...
int PLASMA_dgetri_Tile(PLASMA_desc *A, int *IPIV)
{
    ...
    /* Allocate workspace */
    PLASMA_Alloc_Workspace_dgetri_Tile_Async(A, &descW);
 ...
 *
 *  PLASMA_dgetri_Tile_Async - Computes the inverse of a matrix using the LU
 *  factorization computed by PLASMA_dgetrf.
 *  This method inverts U and then computes inv(A) by solving the system
 *  inv(A)*L = inv(U) for inv(A).
 *  Non-blocking equivalent of PLASMA_dgetri_Tile().
                             ...
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgetri_Tile_Async", "invalid A descriptor");
    ...
    if (plasma_desc_check(W) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgetri_Tile_Async", "invalid W descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descW = *W;
    }
    ...
    plasma_dynamic_call_9(plasma_pdtrsmrv,
