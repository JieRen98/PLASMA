 ...
 * @date 2010-11-15
 * @generated c Fri Apr  1 11:02:53 2016
 *
 **/
#include "common.h"
 ...
 *
 *  PLASMA_claswpc - performs a series of row interchanges on the matrix A.
 ...
 *
 ******************************************************************************/
int PLASMA_claswpc(int N, PLASMA_Complex32_t *A, int LDA,
                  int K1, int K2, const int *IPIV, int INCX)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_claswpc", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (N < 0) {
        plasma_error("PLASMA_claswpc", "illegal value of N");
        return -1;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_claswpc", "illegal value of LDA");
        return -3;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_claswpc", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_cooplap2tile( descA, A, NB, NB, LDA, N, 0, 0, K2, N, sequence, &request,
    ...
    PLASMA_claswpc_Tile_Async(&descA, K1, K2, IPIV, INCX, sequence, &request);
 ...
 *
 *  PLASMA_claswpc_Tile - performs a series of row interchanges on the matrix A.
 *  One row interchange is initiated for each of rows K1 through K2 of A.
 *  Tile equivalent of PLASMA_claswpc().
 ...
 *
 *  PLASMA_claswpc_Tile_Async - performs a series of row interchanges
 *  on the matrix A.  One row interchange is initiated for each of
 *  rows K1 through K2 of A.
 *  Non-blocking equivalent of PLASMA_claswpc_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_claswpc_Tile_Async(PLASMA_desc *A, int K1, int K2, const int *IPIV, int INCX,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if ( (K1 != 1) || (K2 != descA.n) ) {
        plasma_error("PLASMA_claswpc_Tile", "invalid K1 or K2 (1..N is the only interval supported right now)");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
