 ...
 * @ingroup double
 *
 *  PLASMA_dgeadd - Performs a matrix addition similarly to the pdgeadd()
 ...
 *
 ******************************************************************************/
int PLASMA_dgeadd(PLASMA_enum trans, int M, int N,
                  ...
                  double beta,  double *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dgeadd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((trans != PlasmaNoTrans) && (trans != PlasmaTrans) && (trans != PlasmaTrans)) {
        plasma_error("PLASMA_dgeadd", "illegal value of trans");
        return -1;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_dgeadd", "illegal value of M");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_dgeadd", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_dgeadd", "illegal value of LDA");
        return -6;
    }
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_dgeadd", "illegal value of LDB");
        return -9;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgeadd", "plasma_tune() failed");
        return status;
    }
 ...
 * @ingroup double_Tile
 *
 *  PLASMA_dgeadd_Tile - Performs a matrix addition similarly to the pdgeadd()
 *  function from the PBLAS library.
 *  Tile equivalent of PLASMA_dgeadd().
 ...
 *
 ******************************************************************************/
int PLASMA_dgeadd_Tile(PLASMA_enum trans,
                       ...
                       double beta,  PLASMA_desc *B)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dgeadd_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dgeadd_Tile_Async(trans, alpha, A, beta, B, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_dgeadd_Tile_Async - Performs a matrix addition similarly to the
 *  pdgeadd() function from the PBLAS library.
 *  Non-blocking equivalent of PLASMA_dgeadd_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_dgeadd_Tile_Async(PLASMA_enum trans,
                             double alpha, PLASMA_desc *A,
                             double beta,  PLASMA_desc *B,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_dgeadd_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_dgeadd_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_dgeadd_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgeadd_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dgeadd_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    /* Check input arguments */
    if ((trans != PlasmaNoTrans) && (trans != PlasmaTrans) && (trans != PlasmaTrans)) {
        plasma_error("PLASMA_dgeadd_Tile_Async", "illegal value of trans");
    ...
    if ( (Amb != descB.mb) || (Anb != descB.nb) ) {
        plasma_error("PLASMA_dgeadd_Tile_Async", "tile sizes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Am != descB.m) || (An != descB.n) ) {
        plasma_error("PLASMA_dgeadd_Tile_Async", "sizes of matrices have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Ai%Amb != descB.i%descB.mb) ||
         (Aj%Anb != descB.j%descB.nb) )
    {
        plasma_error("PLASMA_dgeadd_Tile_Async", "start indexes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
