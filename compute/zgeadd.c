 ...
 *
 *  PLASMA_zgeadd - Performs a matrix addition similarly to the pzgeadd()
 ...
 *******************************************************************************
 *
 * @sa PLASMA_zgeadd_Tile
 * @sa PLASMA_cgeadd
 * @sa PLASMA_dgeadd
 * @sa PLASMA_sgeadd
 *
 ******************************************************************************/
int PLASMA_zgeadd(PLASMA_enum trans, int M, int N,
                  ...
                  PLASMA_Complex64_t beta,  PLASMA_Complex64_t *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_zgeadd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if ((trans != PlasmaNoTrans) && (trans != PlasmaTrans) && (trans != PlasmaConjTrans)) {
        plasma_error("PLASMA_zgeadd", "illegal value of trans");
        return -1;
    }
    ...
    if (M < 0) {
        plasma_error("PLASMA_zgeadd", "illegal value of M");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_zgeadd", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, Am)) {
        plasma_error("PLASMA_zgeadd", "illegal value of LDA");
        return -6;
    }
    if (LDB < max(1, M)) {
        plasma_error("PLASMA_zgeadd", "illegal value of LDB");
        return -9;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgeadd", "plasma_tune() failed");
        return status;
    }
 ...
 *
 *  PLASMA_zgeadd_Tile - Performs a matrix addition similarly to the pzgeadd()
 *  function from the PBLAS library.
 *  Tile equivalent of PLASMA_zgeadd().
 ...
 *
 * @sa PLASMA_zgeadd
 * @sa PLASMA_zgeadd_Tile_Async
 * @sa PLASMA_cgeadd_Tile
 * @sa PLASMA_dgeadd_Tile
 * @sa PLASMA_sgeadd_Tile
 *
 ******************************************************************************/
int PLASMA_zgeadd_Tile(PLASMA_enum trans,
                       ...
                       PLASMA_Complex64_t beta,  PLASMA_desc *B)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_zgeadd_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_zgeadd_Tile_Async(trans, alpha, A, beta, B, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_zgeadd_Tile_Async - Performs a matrix addition similarly to the
 *  pzgeadd() function from the PBLAS library.
 *  Non-blocking equivalent of PLASMA_zgeadd_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_zgeadd_Tile_Async(PLASMA_enum trans,
                             PLASMA_Complex64_t alpha, PLASMA_desc *A,
                             PLASMA_Complex64_t beta,  PLASMA_desc *B,
                             PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_zgeadd_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_zgeadd_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_zgeadd_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgeadd_Tile_Async", "invalid first descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descA = *A;
    }
    if (plasma_desc_check(B) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgeadd_Tile_Async", "invalid second descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descB = *B;
    }
    /* Check input arguments */
    if ((trans != PlasmaNoTrans) && (trans != PlasmaTrans) && (trans != PlasmaConjTrans)) {
        plasma_error("PLASMA_zgeadd_Tile_Async", "illegal value of trans");
    ...
    if ( (Amb != descB.mb) || (Anb != descB.nb) ) {
        plasma_error("PLASMA_zgeadd_Tile_Async", "tile sizes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Am != descB.m) || (An != descB.n) ) {
        plasma_error("PLASMA_zgeadd_Tile_Async", "sizes of matrices have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if ( (Ai%Amb != descB.i%descB.mb) ||
         (Aj%Anb != descB.j%descB.nb) )
    {
        plasma_error("PLASMA_zgeadd_Tile_Async", "start indexes have to match");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
