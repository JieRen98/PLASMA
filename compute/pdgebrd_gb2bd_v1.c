...
void plasma_pdgebrd_gb2bd_v1(plasma_context_t *plasma)
{
...
#ifdef COMPLEX
            if( standalonework != 0 ) {
                plasma_error("pdgebrd_gb2bd_v1", "standalone is not supported\n");
    ...
    WORK    = (double*) plasma_private_alloc(plasma, NB, PlasmaRealDouble);
                    ...
                    for (k = 1; k <=grsiz; k++){
                        ...
                        if(my_core_id==coreid) {
                            if(myid==1) {
                                ...
                                CORE_dgbtype1cb(uplo, MINMN, NB, A, LDA, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, WANTZ, WORK);
                                ...
                                ss_cond_wait(myid+shift-1, 0, sweepid-1);
                                if(myid%2 == 0){
                                    CORE_dgbtype2cb(uplo, MINMN, NB, A, LDA, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, WANTZ, WORK);
                                }else{
                                    CORE_dgbtype3cb(uplo, MINMN, NB, A, LDA, VQ, TAUQ, VP, TAUP, stind-1, edind-1, sweepid-1, Vblksiz, WANTZ, WORK);
                                }
                                ss_cond_set(myid, 0, sweepid);
                                if(blklastind >= (MINMN-1)) {
