 ...
 * @ingroup float
 *
 *  PLASMA_sgetrf_nopiv - Computes an LU factorization of a general M-by-N matrix A
 ...
 *
 ******************************************************************************/
int PLASMA_sgetrf_nopiv(int M, int N,
                        float *A, int LDA)
{
    int NB;
    int status;
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_nopiv", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_sgetrf_nopiv", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_sgetrf_nopiv", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_sgetrf_nopiv", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sgetrf_nopiv", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_sgetrf_nopiv_Tile_Async(&descA, sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_sgetrf_nopiv_Tile - Computes the tile LU factorization of a matrix.
 *  Tile equivalent of PLASMA_sgetrf_nopiv().  Operates on matrices stored by
...
int PLASMA_sgetrf_nopiv_Tile(PLASMA_desc *A)
{
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_nopiv_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_sgetrf_nopiv_Tile_Async(A, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_sgetrf_nopiv_Tile_Async - Computes the tile LU factorization of a
 *  matrix.  Non-blocking equivalent of PLASMA_sgetrf_nopiv_Tile().  May return
 ...
 *
 ******************************************************************************/
int PLASMA_sgetrf_nopiv_Tile_Async(PLASMA_desc *A,
                                   ...
                                   PLASMA_request *request)
{
    ...
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_nopiv_Tile", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_sgetrf_nopiv_Tile", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_sgetrf_nopiv_Tile", "invalid first descriptor");
    ...
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_sgetrf_nopiv_Tile", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }

    plasma_dynamic_call_3(plasma_psgetrf_nopiv,
        PLASMA_desc, descA,
        PLASMA_sequence*, sequence,
        PLASMA_request*, request);
