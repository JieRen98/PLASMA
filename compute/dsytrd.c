 ...
 *
 * @param[out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_dsytrd
                 ...
                 double *Q, int LDQ)
{
    ...
    PLASMA_dsytrd_Tile_Async(jobz, uplo, &descA, D, E, descT, Q, LDQ, sequence, &request);
                       ...
                       PLASMA_desc *T, double *Q, int LDQ)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_dsytrd_Tile_Async(jobz, uplo, A, D, E, T, Q, LDQ, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
                            ...
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_dsytrd_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.m != descA.n) {
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_dsytrd", "plasma_shared_alloc() failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_dsytrd","pdsytrd_he2hb+pdcopy");
        return status;
    }
    /*=======================================
