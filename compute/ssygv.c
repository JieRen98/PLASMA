                 ...
                 float *Q, int LDQ)
{
    ...
    PLASMA_ssygv_Tile_Async(itype, jobz, uplo,
                            &descA, &descB, W,
                            descT, &descQ,
                            sequence, &request);
                            ...
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    if (plasma_desc_check(T) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_ssygv_Tile_Async", "invalid T descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (plasma_desc_check(Q) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_ssygv_Tile_Async", "invalid Q descriptor");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    } else {
        descQ = *Q;
    }
    ...
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_ssygv_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
     ...
     * Call Eigensolver
     */
    status = PLASMA_ssyev_Tile_Async( jobz, uplo,
                                      A, W, T,
                                      (float*)(Q->mat), Q->lm,
                                      sequence, request );
