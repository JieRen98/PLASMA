 ...
 *
 *  PLASMA_cheevd - Computes all eigenvalues and, optionally,
 ...
 *
 * @param[in, out] descT
 *          On entry, descriptor as return by PLASMA_Alloc_Workspace_cheevd
 ...
 *
 ******************************************************************************/
int PLASMA_cheevd(PLASMA_enum jobz, PLASMA_enum uplo, int N,
                 ...
                 PLASMA_Complex32_t *Q, int LDQ)
{
    ...
    if (plasma == NULL) {
        plasma_error("PLASMA_cheevd", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }

    /* Check input arguments */
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_cheevd", "illegal value of jobz");
        return -1;
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_cheevd", "illegal value of uplo");
        return -2;
    }
    if (N < 0) {
        plasma_error("PLASMA_cheevd", "illegal value of N");
        return -3;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_cheevd", "illegal value of LDA");
        return -5;
    }
    if (LDQ < max(1, N)) {
        plasma_error("PLASMA_cheevd", "illegal value of LDQ");
        return -9;
    }
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cheevd", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_cheevd_Tile_Async(jobz, uplo, &descA, W, descT, Q, LDQ, sequence, &request);
 ...
 *
 *  PLASMA_cheevd_Tile - Computes all eigenvalues and, optionally, eigenvectors of a
 ...
 *
 ******************************************************************************/
int PLASMA_cheevd_Tile(PLASMA_enum jobz, PLASMA_enum uplo,
                      PLASMA_desc *A, float *W,
                      PLASMA_desc *T, PLASMA_Complex32_t *Q, int LDQ)
{
    plasma_context_t *plasma;
    PLASMA_sequence *sequence = NULL;
    PLASMA_request request = PLASMA_REQUEST_INITIALIZER;
    ...
    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cheevd_Tile", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    plasma_sequence_create(plasma, &sequence);
    PLASMA_cheevd_Tile_Async(jobz, uplo, A, W, T, Q, LDQ, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
 ...
 *
 *  PLASMA_cheevd_Tile_Async - Computes all eigenvalues and,
 ...
 *
 ******************************************************************************/
int PLASMA_cheevd_Tile_Async(PLASMA_enum jobz, PLASMA_enum uplo,
                            PLASMA_desc *A,
                            float *W,
                            PLASMA_desc *T,
                            PLASMA_Complex32_t *Q, int LDQ,
                            PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    float *E;
    int N, NB, LDAB, status;
    plasma_time_t timelpk, timeaplQ2, timeT;
    plasma_time_t timeB, timeblg, timeaplQ1, timeconv1, timeconv2, timeall;

    plasma = plasma_context_self();
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_cheevd_Tile_Async", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    if (sequence == NULL) {
        plasma_fatal_error("PLASMA_cheevd_Tile_Async", "NULL sequence");
        return PLASMA_ERR_UNALLOCATED;
    }
    if (request == NULL) {
        plasma_fatal_error("PLASMA_cheevd_Tile_Async", "NULL request");
        return PLASMA_ERR_UNALLOCATED;
    }
    ...
    if (plasma_desc_check(A) != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cheevd_Tile_Async", "invalid descriptor");
    ...
    /* Check input arguments */
    if (jobz != PlasmaNoVec && jobz != PlasmaVec) {
        plasma_error("PLASMA_cheevd_Tile_Async", "illegal value of jobz");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (uplo != PlasmaLower && uplo != PlasmaUpper) {
        plasma_error("PLASMA_cheevd_Tile_Async", "illegal value of uplo");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.m != descA.n) {
        plasma_error("PLASMA_cheevd_Tile_Async", "matrix need to be square");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    if (descA.nb != descA.mb) {
        plasma_error("PLASMA_cheevd_Tile_Async", "only square tiles supported");
        return plasma_request_fail(sequence, request, PLASMA_ERR_ILLEGAL_VALUE);
    }
    ...
    if (AB == NULL) {
        plasma_error("PLASMA_cheevd_Tile_Async", "plasma_shared_alloc(AB) failed");
        plasma_shared_free(plasma, AB);
        return PLASMA_ERR_OUT_OF_RESOURCES;
    }
    E = (float *)plasma_shared_alloc(plasma, N, PlasmaRealDouble);
    if (E == NULL) {
        plasma_error("PLASMA_cheevd_Tile_Async", "plasma_shared_alloc(E) failed");
    ...
    status = sequence->status;
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_cheevd","pchetrd_he2hb+pccopy");
        return status;
    }
    /*=======================================
    ...
    if( jobz == PlasmaVec ) {
        ...
        if ( (TAU2 == NULL) || (V2 == NULL) || (T2 == NULL) ) {
            plasma_error("PLASMA_cheevd", "plasma_shared_alloc() failed");
    ...
    plasma_static_call_13(plasma_pchetrd_hb2st_v1,
        PLASMA_enum,         blguplo,
        int,                 N,
        int,                 NB,
        int,                 Vblksiz,
     ...
     *  calling eigensolver
     *=======================================*/
#if defined(LAPACK_HEEVDC)
    /* call eigensolver using lapack routine for our resulting tridiag [D E] */
    plasma_setlapack_multithreads(plasma->world_size);
    /*
    status = LAPACKE_cstevd( LAPACK_COL_MAJOR, lapack_const(jobz), N, W, E, Q, LDQ );
    */
    if(jobz == PlasmaNoVec){
        status = LAPACKE_cstedc( LAPACK_COL_MAJOR, lapack_const(PlasmaNoVec),
                        N, W, E, Q, LDQ );
    } else {
        status = LAPACKE_cstedc( LAPACK_COL_MAJOR, lapack_const(PlasmaIvec),
                        N, W, E, Q, LDQ );
    }
    plasma_setlapack_sequential(plasma);
#else
    {
        PLASMA_enum jobz_stedc = (jobz == PlasmaVec) ? PlasmaIvec : jobz;
        status = PLASMA_cstedc_Async(jobz_stedc, N, W, E, Q, LDQ, sequence, request);
    }
#endif
    ...
    if (jobz == PlasmaVec) {
        ...
        if( NB < N ){
            ...
            if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
                plasma_cooplap2tile( descQ, Q, NB, NB, LDQ, N, 0, 0, N, N, sequence, request, plasma_desc_mat_free(&(descQ)) );
            }else {
                plasma_ciplap2tile( descQ, Q, NB, NB, LDQ, N, 0, 0, N, N, sequence, request );
            }
