...
#define Acpy(__m) (BLKADDR(W, PLASMA_Complex32_t, (__m), 0))
#define IPIV(k)    (&(IPIV[(int64_t)A.mb*(int64_t)(k)]))
#define RANK(__m, __k) (&(Wi[(int64_t)W.mb*(int64_t)(__m) + (int64_t)W.lm*(__k)]))

void plasma_pcgetrf_rectil_panel_quark(plasma_context_t *plasma,
                                       int *panel_thread_count,
                                       CORE_cgetrf_data_t *data, PLASMA_desc A, int *IPIV,
                                       ...
                                       PLASMA_sequence *sequence, PLASMA_request *request)
{
    ...
    QUARK_CORE_cgetrf_rectil(
        ...
        data, A, A(0, 0), A.mb*A.nb, IPIV,
        sequence, request, 1, A.i,
        *panel_thread_count );
}

void plasma_pcgetrf_tntpiv_panel_quark(plasma_context_t *plasma,
...
void plasma_pcgetrf_tntpiv_qrrr_panel_quark(plasma_context_t *plasma,
                                            PLASMA_desc A, int *IPIV,
                                            PLASMA_desc W, int *Wi,
                                            Quark_Task_Flags *task_flags,
                                            PLASMA_sequence *sequence, PLASMA_request *request)
 ...
 **/
void plasma_pcgetrf_tntpiv_quark(PLASMA_desc A, int *IPIV,
                                 PLASMA_desc W, int *Wi,
                                 PLASMA_sequence *sequence, PLASMA_request *request)
{
    int k, m, n, minmnt;
    ...
    plasma_profile_by_function( &task_flags, GETRF );

    if (PLASMA_TNT_MODE == PLASMA_TOURNAMENT_LU)
        panel = plasma_pcgetrf_tntpiv_panel_quark;
    else
        panel = plasma_pcgetrf_tntpiv_qrrr_panel_quark;
