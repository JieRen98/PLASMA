...
void plasma_pchetrd_hb2st_v1(plasma_context_t *plasma)
{
...
#ifdef COMPLEX
            for (i=0; i<N; i++){
                D[i] = cabsf(A[LDA*i]); /* A is band storage*/
            }
#else
            for (i=0; i<N; i++){
                D[i] = A[LDA*i];
    ...
    if ( NB == 1 ) {
        if( my_core_id == 0 ) {
            ...
            if( standalonework != 0 ) {
                 ...
                 */
                if( uplo == PlasmaLower ) {
                    PLASMA_Complex32_t ztmp, absztmp;
                    ...
                    for (i=0; i<N-1; i++) {
                        ...
                        E[i]       = absztmp;
                        if( absztmp != 0. )
                            ztmp = (PLASMA_Complex32_t) (ztmp / (PLASMA_Complex32_t) absztmp);
                        else
                            ztmp = (PLASMA_Complex32_t)1.;
                        if( i < N-2 ) {
                            A[LDA*(i+1)+1] *= ztmp;
                        }
                    ...
                    D[N-1] = crealf(A[LDA*(N-1)]);
                }
                /*
                 * PlasmaUpper
                 */
                else {
                    plasma_error("pchetrd_hb2st_v1", "PlasmaUpper is not supported\n");
                    plasma_request_fail(sequence, request, PLASMA_ERR_NOT_SUPPORTED);
                    return;
                }
            }
...
#endif
            {
                ...
                if( uplo == PlasmaLower ){
                    for (i=0; i < N-1; i++) {
                        D[i] = crealf(A[i*LDA]);
                        E[i] = crealf(A[i*LDA+1]);
                    }
                    D[N-1] = crealf(A[(N-1)*LDA]);
                }
                else {
                    for (i=0; i < N-1; i++) {
                        D[i] = crealf(A[i*LDA+NB]);
                        E[i] = crealf(A[i*LDA+NB-1]);
                    }
                    D[N-1] = crealf(A[(N-1)*LDA+NB]);
                    E[N-1] = 0.;
                }
            }
        }
    ...
    WORK    = (PLASMA_Complex32_t*) plasma_private_alloc(plasma, NB, PlasmaComplexFloat);
                    ...
                    for (k = 1; k <=grsiz; k++){
                        ...
                        if(my_core_id==coreid) {
                            if(myid==1) {
                                ...
                                CORE_chbtype1cb(N, NB, A, LDA, V, TAU, stind-1, edind-1, sweepid-1, Vblksiz, WANTZ, WORK);
                                ...
                                ss_cond_wait(myid+shift-1, 0, sweepid-1);
                                if(myid%2 == 0)
                                    CORE_chbtype2cb(N, NB, A, LDA, V, TAU, stind-1, edind-1, sweepid-1, Vblksiz, WANTZ, WORK);
                                else
                                    CORE_chbtype3cb(N, NB, A, LDA, V, TAU, stind-1, edind-1, sweepid-1, Vblksiz, WANTZ, WORK);
 ...
 **/
/***************************************************************************/
void plasma_pchetrd_hb2st_v1_quark(PLASMA_enum uplo, int N, int NB, int Vblksiz,
