 ...
 *
 *  PLASMA_zgetrf_nopiv - Computes an LU factorization of a general M-by-N matrix A
 ...
 * @sa PLASMA_zgetrf_nopiv_Tile_Async
                        ...
                        PLASMA_Complex64_t *A, int LDA)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_zgetrf_nopiv", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (M < 0) {
        plasma_error("PLASMA_zgetrf_nopiv", "illegal value of M");
        return -1;
    }
    if (N < 0) {
        plasma_error("PLASMA_zgetrf_nopiv", "illegal value of N");
        return -2;
    }
    if (LDA < max(1, M)) {
        plasma_error("PLASMA_zgetrf_nopiv", "illegal value of LDA");
        return -4;
    }
    /* Quick return */
    if (min(M, N) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_zgetrf_nopiv", "plasma_tune() failed");
        return status;
    }
    ...
    PLASMA_zgetrf_nopiv_Tile_Async(&descA, sequence, &request);
 ...
 *
 *  PLASMA_zgetrf_nopiv_Tile - Computes the tile LU factorization of a matrix.
 *  Tile equivalent of PLASMA_zgetrf_nopiv().  Operates on matrices stored by
...
int PLASMA_zgetrf_nopiv_Tile(PLASMA_desc *A)
{
    ...
    plasma_sequence_create(plasma, &sequence);
    PLASMA_zgetrf_nopiv_Tile_Async(A, sequence, &request);
    plasma_dynamic_sync();
    status = sequence->status;
    plasma_sequence_destroy(plasma, sequence);
    return status;
