 ...
 * @ingroup float
 *
 *  PLASMA_strsmrv - Computes triangular solve A*X = B or X*A = B.
 *
 *******************************************************************************
 *
 * @param[in] side
 ...
 *
 ******************************************************************************/
int PLASMA_strsmrv(PLASMA_enum side, PLASMA_enum uplo,
                 ...
                 float *B, int LDB)
{
    ...
    if (plasma == NULL) {
        plasma_fatal_error("PLASMA_strsmrv", "PLASMA not initialized");
        return PLASMA_ERR_NOT_INITIALIZED;
    }
    /* Check input arguments */
    if (side != PlasmaLeft && side != PlasmaRight) {
        plasma_error("PLASMA_strsmrv", "illegal value of side");
        return -1;
    }
    if (uplo != PlasmaUpper && uplo != PlasmaLower) {
        plasma_error("PLASMA_strsmrv", "illegal value of uplo");
        return -2;
    }
    if (transA != PlasmaTrans && transA != PlasmaNoTrans && transA != PlasmaTrans ) {
        plasma_error("PLASMA_strsmrv", "illegal value of transA");
        return -3;
    }
    if (diag != PlasmaUnit && diag != PlasmaNonUnit) {
        plasma_error("PLASMA_strsmrv", "illegal value of diag");
        return -4;
    }
    if (N < 0) {
        plasma_error("PLASMA_strsmrv", "illegal value of N");
        return -5;
    }
    if (NRHS < 0) {
        plasma_error("PLASMA_strsmrv", "illegal value of NRHS");
        return -6;
    }
    if (LDA < max(1, N)) {
        plasma_error("PLASMA_strsmrv", "illegal value of LDA");
        return -8;
    }
    if (LDB < max(1, N)) {
        plasma_error("PLASMA_strsmrv", "illegal value of LDB");
        return -10;
    }
    /* Quick return */
    if (min(N, NRHS) == 0)
    ...
    if (status != PLASMA_SUCCESS) {
        plasma_error("PLASMA_strsmrv", "plasma_tune() failed");
        return status;
    }
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        plasma_sooplap2tile( descA, A, NB, NB, LDA, NA,   0, 0, NA, NA,    sequence, &request,
    ...
    if ( PLASMA_TRANSLATION == PLASMA_OUTOFPLACE ) {
        ...
        plasma_siptile2lap( descA, A, NB, NB, LDA, NA,    sequence, &request);
 ...
 * @ingroup float_Tile
 *
 *  PLASMA_strsmrv_Tile - Computes triangular solve.
 *  Tile equivalent of PLASMA_strsmrv().
 ...
 *
 ******************************************************************************/
int PLASMA_strsmrv_Tile(PLASMA_enum side, PLASMA_enum uplo,
                      PLASMA_enum transA, PLASMA_enum diag,
 ...
 *
 *  PLASMA_strsmrv_Tile_Async - Computes triangular solve.
 *  Non-blocking equivalent of PLASMA_strsmrv_Tile().
 ...
 *
 ******************************************************************************/
int PLASMA_strsmrv_Tile_Async(PLASMA_enum side, PLASMA_enum uplo,
                            PLASMA_enum transA, PLASMA_enum diag,
                            float alpha, PLASMA_desc *A, PLASMA_desc *B,
